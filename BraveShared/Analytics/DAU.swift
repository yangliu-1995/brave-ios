...
public class DAU {
    ...
    private static let baseUrl =
        "https://laptop-updates.brave.com/\(apiVersion)/usage/ios?platform=ios"
    ...
    public init(date: Date = Date()) {
        today = date
        apiKey = (Bundle.main.infoDictionary?[Self.apiKeyPlistKey] as? String)?.trimmingCharacters(
    ...
    func weekOfInstallationParam(for woi: String? = Preferences.DAU.weekOfInstallation.value)
        -> URLQueryItem
    {
        var woi = woi
    ...
    func dtoiParam() -> URLQueryItem {
        ...
        return URLQueryItem(
            name: paramName,
            value: DAU.dateFormatter.string(from: installationDate)
        )
    }
    ...
    private func getPings(forDate date: Date, lastPingDate: Date) -> Set<PingType> {
        ...
        if let nextFirstOfMonth = nextDate(matching: DateComponents(day: 1)),
