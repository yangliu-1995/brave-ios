...
public class UserReferralProgram {
    ...
    public init?() {
        ...
        guard
            let apiKey = getPlistString(for: UserReferralProgram.apiKeyPlistKey)?
                .trimmingCharacters(
    ...
    public func referralLookup(
        ...
        let referralBlock: (ReferralData?, UrpError?) -> Void = { [weak self] referral, error in
            ...
            if ref.isExtendedUrp() {
                if let headers = ref.customHeaders {
                    do {
                        try Preferences.URP.customHeaderData.value = NSKeyedArchiver.archivedData(
                            withRootObject: headers,
                            requiringSecureCoding: false
                        )
                    } catch {
                        log.error(
                            "Failed to save URP custom header data \(headers) with error: \(error.localizedDescription)"
                        )
                    }
                }
                ...
                UrpLog.log(
                    "Extended referral code found, opening landing page: \(ref.offerPage ?? "404")"
                )
        ...
        service.referralCodeLookup(
            refCode: UserReferralProgram.getReferralCode(),
            completion: referralBlock
        )
    }
    ...
    public class func getReferralCode() -> String? {
            ...
            Date().timeIntervalSince1970 >= referralCodeDeleteDate
        {
            ...
            if Preferences.URP.referralCodeDeleteDate.value == nil {
                ...
                Preferences.URP.referralCodeDeleteDate.value =
                    Date().timeIntervalSince1970 + timeToDelete
            }
    ...
    public class func shouldAddCustomHeader(for request: URLRequest) -> (
        value: String, field: String
    )? {
        guard let customHeaders = UserReferralProgram.shared?.customHeaders,
            let hostUrl = request.url?.host
        else { return nil }
        ...
        for customHeader in customHeaders {
            ...
            for domain in customHeader.domainList where hostUrl.contains(domain) {
                ...
                if !cookieOnly, let allFields = request.allHTTPHeaderFields,
                    !allFields.keys.contains(customHeader.headerField)
                {
