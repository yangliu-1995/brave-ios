...
public class PinningCertificateEvaluator: NSObject, URLSessionDelegate {
    ...
    public init(
        ...
        options: PinningOptions = [.default, .validateHost]
    ) {
    ...
    private func error(reason: String) -> NSError {
        return NSError(
            domain: "com.brave.pinning-certificate-evaluator",
            code: -1,
    ...
    public func evaluate(_ trust: SecTrust, forHost host: String) throws {
        ...
        if options.contains(.allowSelfSigned) {
            guard SecTrustSetAnchorCertificates(trust, certificates as CFArray) == errSecSuccess
            else {
        ...
        let clientCertificates = Set(
            certificates.compactMap({ SecCertificateCopyData($0) as Data })
        )
