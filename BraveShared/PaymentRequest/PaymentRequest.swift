...
public struct PaymentRequest: Decodable {
    ...
    public struct Details: Decodable {
        ...
        public init(from decoder: Decoder) throws {
            ...
            skuTokens = try container.decode(String.self, forKey: .id).split(separator: ";").map(
                String.init
            )
    ...
    public init(from decoder: Decoder) throws {
        ...
        methodData = try JSONDecoder().decode(
            [MethodData].self,
            from: methodDataString.data(using: String.Encoding.utf8) ?? Data()
        )
        details = try JSONDecoder().decode(
            Details.self,
            from: detailsString.data(using: String.Encoding.utf8) ?? Data()
        )
