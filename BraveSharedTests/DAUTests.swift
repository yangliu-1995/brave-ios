...
class DAUTests: XCTestCase {
    ...
    private let dau = DAU(date: Date(timeIntervalSince1970: 1_183_809_600))
    ...
    private func pingWithDateAndCompare(
        ...
        XCTAssert(
            params!.queryParams.contains(URLQueryItem(name: "daily", value: daily.description))
        )
        XCTAssert(
            params!.queryParams.contains(URLQueryItem(name: "weekly", value: weekly.description))
        )
        XCTAssert(
            params!.queryParams.contains(URLQueryItem(name: "monthly", value: monthly.description))
        )
        XCTAssert(
            params!.queryParams.contains(URLQueryItem(name: "first", value: first.description))
        )
