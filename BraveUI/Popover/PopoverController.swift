...
public class PopoverController: UIViewController {
    ...
    override public func viewDidLoad() {
        ...
        case .preferredContentSize:
            containerViewHeightConstraint = containerView.heightAnchor.constraint(
                equalToConstant: contentController.preferredContentSize.height
                    + PopoverUX.arrowSize.height
            )
    ...
    override public func preferredContentSizeDidChange(
        ...
        if case .preferredContentSize = contentSizeBehavior {
            ...
            size.height = min(
                size.height,
                UIScreen.main.bounds.height - containerView.frame.origin.y
            ...
            self.containerViewHeightConstraint?.springAnimate(
                property: kPOPLayoutConstraintConstant,
                key: "constant"
            ) { animation, _ in
                animation.toValue = size.height + PopoverUX.arrowSize.height
                animation.animationDidApplyBlock = { _ in
                    if let nc = self.contentController as? UINavigationController {
                        nc.viewControllers.filter { $0.isViewLoaded }.forEach {
                            $0.view.frame = nc.view.bounds
                        }
                    }
            ...
            self.containerViewWidthConstraint?.springAnimate(
                property: kPOPLayoutConstraintConstant,
                key: "constant"
            ) { animation, _ in
    ...
    private func anchorPointDelta(from context: PresentationContext, popoverRect rect: CGRect)
        -> CGPoint
    {
    ...
    public func present(from view: UIView, on viewController: UIViewController) {
        let convertedOriginViewCenter = viewController.view.convert(
            view.center,
            from: view.superview
        )
        ...
        let constrainedWidth =
            viewController.view.bounds.width - outerMargins.left - outerMargins.right
        ...
        case .autoLayout:
            contentSize = contentController.view.systemLayoutSizeFitting(
                CGSize(
                    ...
                    height: viewController.view.bounds.height - outerMargins.top
                        - outerMargins.bottom
                )
            )
        case .preferredContentSize:
...
extension PopoverController {
    ...
    @objc private func pannedPopover(_ pan: UIPanGestureRecognizer) {
        ...
        if pan.state == .ended {
            ...
            if contentController.popoverShouldDismiss(self)
                && (passedVelocityThreshold || scale < 0.5)
            {
                dismiss(animated: true)
            } else {
                UIView.animate(
...
extension PopoverController: BasicAnimationControllerDelegate {
    ...
    public func animatePresentation(context: UIViewControllerContextTransitioning) {
        ...
        containerView.snp.makeConstraints {
            ...
            $0.top.greaterThanOrEqualTo(self.view.safeAreaLayoutGuide.snp.top).offset(
                outerMargins.top
            )
            $0.bottom.lessThanOrEqualTo(self.view.safeAreaLayoutGuide.snp.bottom).offset(
                -outerMargins.bottom
            )
            $0.left.greaterThanOrEqualTo(self.view.safeAreaLayoutGuide.snp.left).offset(
                outerMargins.left
            )
        ...
        containerView.springAnimate(property: kPOPViewAlpha, key: "alpha") {
            animation,
            inProgress in
            animation.toValue = 1.0
        ...
        containerView.arrowOrigin = CGPoint(
            x: containerView.bounds.midX + translationDelta.x,
            y: 0.0
        )
