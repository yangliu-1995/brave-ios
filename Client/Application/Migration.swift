...
class Migration {
    ...
    static func moveDatabaseToApplicationDirectory() {
        ...
        if Preferences.Database.DocumentToSupportDirectoryMigration.previousAttemptedVersion.value
            == AppInfo.appVersion
        {
        ...
        Preferences.Database.DocumentToSupportDirectoryMigration.previousAttemptedVersion.value =
            AppInfo.appVersion
    }
    ...
    private static func documentsDirectoryCleanup() {
        ...
        FileManager.default.removeFolder(
            withName: "https-everywhere-data",
            location: .documentDirectory
        )

        FileManager.default.moveFile(
            sourceName: "CookiesData.json",
            sourceLocation: .documentDirectory,
...
extension Preferences {
    ...
    fileprivate class func migratePreferences(keyPrefix: String) {
        ...
        migrate(
            key: "search.suggestions.showOptIn",
            to: Preferences.Search.shouldShowSuggestionsOptIn
        )
        ...
        let baseDir = NSSearchPathForDirectoriesInDomains(.libraryDirectory, .userDomainMask, true)[
            0
        ]
        ...
        if let pinLockInfo = KeychainWrapper.standard.object(forKey: "pinLockInfo")
            as? AuthenticationKeychainInfo
        {
