...
public struct DeviceCheckRegistration: Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        guard
            let data = Data(
                base64Encoded: try container.decode(String.self, forKey: .enrollmentBlob)
            )
        else {
            throw NSError(
                domain: "com.brave.device.check.enrollment",
                code: -1,
                userInfo: [
...
public struct AttestationVerifcation: Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        guard
            let data = Data(
                base64Encoded: try container.decode(String.self, forKey: .attestationBlob)
            )
        else {
            throw NSError(
                domain: "com.brave.device.check.enrollment",
...
public class DeviceCheckClient {
    ...
    public func registerDevice(
        enrollment: DeviceCheckRegistration,
    ...
    public func getAttestation(
        attestation: Attestation,
        _ completion: @escaping (AttestationBlob?, Error?) -> Void
    ) {
        do {
            try executeRequest(.getAttestation(attestation)) {
                (result: Swift.Result<AttestationBlob, Error>) in
                switch result {
    ...
    public func setAttestation(
        ...
        do {
            try executeRequest(.setAttestation(nonce, verification)) {
                (result: Swift.Result<Data, Error>) in
                if case .failure(let error) = result {
                    return completion(error)
                }
    ...
    public func generateEnrollment(
        ...
        do {
            guard let privateKey = try Cryptography.generateKey(id: DeviceCheckClient.privateKeyId)
            else {
            ...
            let signature = try privateKey.sign(message: enrollment.bsonData())
                .base64EncodedString()
    ...
    public func generateAttestation(paymentId: String, _ completion: (Attestation?, Error?) -> Void) {
        do {
            guard
                let privateKey = try Cryptography.getExistingKey(id: DeviceCheckClient.privateKeyId)
            else {
    ...
    public func generateAttestationVerification(
        nonce: String,
        _ completion: (AttestationVerifcation?, Error?) -> Void
    ) {
        do {
            guard
                let privateKey = try Cryptography.getExistingKey(id: DeviceCheckClient.privateKeyId)
            ...
            let signature = try privateKey.sign(message: try attestation.bsonData())
                .base64EncodedString()
    ...
    private enum Request {
        ...
        func url(for client: DeviceCheckClient) -> URL? {
            ...
            case .setAttestation(let nonce, _):
                let nonce =
                    nonce.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? nonce
                return URL(
                    string: "/v1/devicecheck/attestations/\(nonce)",
    ...
    private func executeRequest<T: Codable>(
        ...
        let task = URLSession(configuration: .ephemeral, delegate: nil, delegateQueue: .main)
            .dataTask(
                ...
                if let data = data,
                    let error = try? JSONDecoder().decode(DeviceCheckError.self, from: data)
                {
                    return completion(.failure(error))
                }
                ...
                guard let data = data else {
                    return completion(
                        .failure(
                            DeviceCheckError(
                                message: "Validation Failed: Empty Server Response",
                                code: 500
                            )
                        )
                    )
