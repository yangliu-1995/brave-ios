...
class PaymentRequestExtension: NSObject {
    ...
    init(rewards: BraveRewards, tab: Tab, paymentRequested: @escaping PaymentRequestHandler) {
        ...
        token = UserScriptManager.securityToken.uuidString.replacingOccurrences(
            of: "-",
            with: "",
            options: .literal
        )
...
extension PaymentRequestExtension: TabContentScript {
    ...
    func userContentController(
        ...
        do {
            ...
            guard body.methodData.contains(where: { $0.supportedMethods.lowercased() == "bat" })
            else {
                sendPaymentRequestError(
            ...
            guard
                body.details.displayItems.map({ $0.amount.currency }).allSatisfy({
                    $0 == body.details.total.amount.currency
                })
            else {
                sendPaymentRequestError(
            ...
            guard
                Double(body.details.total.amount.value)
                    == body.details.displayItems.compactMap({ (Double($0.amount.value)) }).reduce(
                        0,
                        +
                    )
            else {
            ...
            paymentRequested(body) { response in
                ...
                case .completed(let orderId):
                    ensureMainThread {
                        self.tab?.webView?.evaluateSafeJavaScript(
                            functionName:
                                "PaymentRequestCallback\(self.token).paymentreq_postCreate",
                            args: [orderId, "", ""]
                        ) { _, error in
                            if error != nil {
                                log.error(error)
