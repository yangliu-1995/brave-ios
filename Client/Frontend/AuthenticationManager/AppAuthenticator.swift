...
class AppAuthenticator {
    ...
    init(
        protectedWindow: UIWindow,
        promptImmediately: Bool,
        isPasscodeEntryCancellable: Bool = true
    ) {
    ...
    private func promptUserForAuthentication() {
        guard let authInfo = KeychainWrapper.sharedAppContainerKeychain.authenticationInfo(),
            !isPrompting
        else { return }
            ...
            cancel: {
                if self.isPasscodeEntryCancellable {
                    ...
                    self.presentPasscodeAuthentication(
                        self.blurController,
                        delegate: self,
                        isCancellable: self.isPasscodeEntryCancellable
                    )
                }
            },
            fallback: {
...
extension AppAuthenticator {
    private static func presentAuthenticationUsingInfo(
        _ authenticationInfo: AuthenticationKeychainInfo,
        ...
        if authenticationInfo.useTouchID {
            ...
            localAuthContext.evaluatePolicy(
                ...
                guard let authError = error,
                    let code = LAError.Code(rawValue: authError._code)
                else {
                    return
                }
                ...
                DispatchQueue.main.async {
                    switch code {
                    case .userFallback, .biometryNotEnrolled, .biometryNotAvailable,
                        .biometryLockout:
                        fallback?()
                    case .userCancel:
                        cancel?()
    ...
    private func presentPasscodeAuthentication(
        ...
        delegate: PasscodeEntryDelegate?,
        isCancellable: Bool
    ) {
