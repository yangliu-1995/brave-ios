...
class AdContentButton: UIControl {
    ...
    public override var isHighlighted: Bool {
        didSet {
            ...
            self.layer.springAnimate(property: kPOPLayerScaleXY, key: "isHighlighted") {
                animation,
                _ in
    ...
    func applyTheme(for traitCollection: UITraitCollection) {
        ...
        appNameLabel.appearanceTextColor = (isDarkMode ? UIColor.white : UIColor.black)
            .withAlphaComponent(0.5)
        titleLabel.appearanceTextColor = isDarkMode ? .white : .black
        bodyLabel.appearanceTextColor = isDarkMode ? .white : .black
        backgroundView.contentView.backgroundColor =
            isDarkMode
            ? UIColor.black.withAlphaComponent(0.3) : UIColor.white.withAlphaComponent(0.7)
    }
