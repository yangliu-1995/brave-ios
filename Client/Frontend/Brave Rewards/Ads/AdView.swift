...
public class AdView: UIView {
    ...
    let openSwipeButton = AdSwipeButton(contentType: .text(Strings.Ads.open, textColor: .white))
        .then {
            $0.backgroundColor = Colors.blurple500
        }
    let dislikeSwipeButton = AdSwipeButton(
        contentType: .image(UIImage(imageLiteralResourceName: "dislike-ad-icon"))
    ).then {
    ...
    func setSwipeTranslation(
        ...
        if animated {
            adContentButton.layer.springAnimate(
                property: kPOPLayerTranslationX,
                key: "translation.x"
            ) {
                animation,
                _ in
