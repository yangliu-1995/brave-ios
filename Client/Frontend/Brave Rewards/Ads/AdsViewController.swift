...
public class AdsViewController: UIViewController {
    ...
    public func display(
        ...
        adView.adContentButton.addTarget(
            self,
            action: #selector(touchDownAdView(_:)),
            for: .touchDown
        )
        adView.adContentButton.addTarget(
            self,
            action: #selector(touchUpOutsideAdView(_:)),
            for: .touchUpOutside
        )
        adView.adContentButton.addTarget(
            self,
            action: #selector(tappedAdView(_:)),
            for: .touchUpInside
        )
        adView.openSwipeButton.addTarget(
            self,
            action: #selector(tappedOpen(_:)),
            for: .touchUpInside
        )
        adView.dislikeSwipeButton.addTarget(
            self,
            action: #selector(tappedDisliked(_:)),
            for: .touchUpInside
        )
        let dismissPanGesture = UIPanGestureRecognizer(
            target: self,
            action: #selector(dismissPannedAdView(_:))
        )
        dismissPanGesture.name = dismissGestureName
        dismissPanGesture.delegate = self
        adView.addGestureRecognizer(dismissPanGesture)

        let swipePanGesture = UIPanGestureRecognizer(
            target: self,
            action: #selector(swipePannedAdView(_:))
        )
    ...
    @objc private func dismissPannedAdView(_ pan: UIPanGestureRecognizer) {
        ...
        case .cancelled:
            ...
            setupTimeoutTimer(for: adView)
            adView.layer.springAnimate(property: kPOPLayerTranslationY, key: "translation.y") {
                animation,
                _ in
                animation.toValue = 0
            }
