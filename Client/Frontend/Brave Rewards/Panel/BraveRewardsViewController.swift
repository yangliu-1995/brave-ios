    ...
    private var publisher: PublisherInfo? {
        didSet {
            ...
            rewardsView.publisherView.bodyLabel.text =
                isVerified
                ? Strings.Rewards.supportingPublisher : Strings.Rewards.unverifiedPublisher
            if !isVerified {
                rewardsView.publisherView.faviconImageView.clearMonogramFavicon()
                rewardsView.publisherView.faviconImageView.image = #imageLiteral(
                    resourceName: "rewards-panel-unverified-pub"
    ...
    override func viewDidLoad() {
        ...
        if !rewards.isEnabled {
            ...
            if let url = tab.url, !url.isLocal {
                ...
                rewards.ledger.fetchPublisherActivity(
                    from: url,
                    faviconURL: nil,
                    publisherBlob: nil,
            ...
            let legacyWallet = legacyWallet
        {
            legacyWallet.updateDrainStatus { status in
                ...
                self.rewardsView.legacyWalletTransferStatusButton.titleLabel.text =
                    status?.statusButtonTitle
        ...
        rewardsView.legacyWalletTransferButton.addTarget(
            self,
            action: #selector(tappedRewardsTransfer),
            for: .touchUpInside
        )
        rewardsView.legacyWalletTransferButton.dismissButton.addTarget(
            self,
            action: #selector(tappedDismissRewardsTransfer),
            for: .touchUpInside
        )
        rewardsView.legacyWalletTransferStatusButton.addTarget(
            self,
            action: #selector(tappedRewardsStatusButton),
            for: .touchUpInside
        )
        rewardsView.legacyWalletTransferStatusButton.dismissButton.addTarget(
            self,
            action: #selector(tappedDismissTransferStatus),
            for: .touchUpInside
        )
        ...
        if !AppConstants.buildChannel.isPublic {
            let tapGesture = UITapGestureRecognizer(
                target: self,
                action: #selector(tappedHostLabel(_:))
            )
    ...
    @objc private func rewardsToggleValueChanged() {
        ...
        if publisher != nil {
            UIView.animate(withDuration: 0.15) {
                ...
                self.rewardsView.publisherView.alpha =
                    self.rewardsView.rewardsToggle.isOn ? 1.0 : 0.0
            }
        }
    }
