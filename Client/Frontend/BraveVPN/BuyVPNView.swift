...
extension BuyVPNViewController {
    class View: UIView {
        ...
        private lazy var vpnPlansStackView = UIStackView().then { stackView in
            ...
            let poweredByView =
                BraveVPNCommonUI.Views.poweredByView(
                    textColor: .white,
                    fontSize: 15,
                    imageColor: .white
                )
        ...
        override init(frame: CGRect) {
            ...
            let checkmarkSlices = checkmarkViewStrings.splitEvery(
                CheckmarksView.maxCheckmarksPerView
            )
        ...
        override func layoutSubviews() {
            ...
            checkmarksStackView.bounds = CGRect(
                origin: .zero,
                size: checkmarksStackView.systemLayoutSizeFitting(
...
extension BuyVPNViewController {
    class SubscriptionButton: UIControl {
        ...
        private struct UX {
            static let disclaimerColor = #colorLiteral(
                red: 0.07058823529,
                green: 0.6392156863,
                blue: 0.4705882353,
                alpha: 1
            )
            static let primaryTextColor = #colorLiteral(
                red: 1,
                green: 1,
                blue: 0.9411764706,
                alpha: 1
            ...
            case .yearly:
                ...
                guard let monthlyProduct = VPNProductInfo.monthlySubProduct,
                    ...
                    let formattedYearlyPrice =
                        yearlyProduct.price.frontSymbolCurrencyFormatted(
                            with: yearlyProduct.priceLocale
                        )
                else {
                    break
                }
            ...
            let priceStackView = UIStackView().then { stackView in
                ...
                if let priceDiscount = priceDiscount {
                    let discountLabel = BraveVPNCommonUI.Views.ShrinkableLabel().then {
                        let strikeThroughText = NSMutableAttributedString(string: priceDiscount)
                            .then {
                                $0.addAttribute(
                        ...
                        $0.appearanceTextColor = #colorLiteral(
                            red: 1,
                            green: 1,
                            blue: 0.9411764706,
                            alpha: 1
