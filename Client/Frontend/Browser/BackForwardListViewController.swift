    ...
    UIGestureRecognizerDelegate
{
    ...
    var tableHeight: CGFloat {
        ...
        return min(
            BackForwardViewUX.rowHeight * CGFloat(listData.count),
    ...
    func homeAndNormalPagesOnly(_ bfList: WKBackForwardList) {
        ...
        listData = items.filter({
            return !($0.url.isLocal && ($0.url.originalURLFromErrorURL?.isLocal ?? true))
                || $0.url.isAboutHomeURL
        })
    }
            ...
            if snappedToBottom {
                ...
                verticalConstraints += [
                    make.top.equalTo(self.view).offset(
                        bvc.header.frame.height + UIApplication.shared.statusBarFrame.size.height
                    ).constraint
                ]
            }
        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let urlString =
            item.url.isLocal ? item.url.getQuery()["url"]?.unescape() : item.url.absoluteString
