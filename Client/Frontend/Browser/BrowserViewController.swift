...
class BrowserViewController: UIViewController {
    ...
    fileprivate func didInit() {
        ...
        rewardsEnabledObserveration = rewards.observe(\.isEnabled, options: [.new]) {
            [weak self] _, _ in
            ...
            if action == .opened {
                ...
                if url == nil,
                    let percentEncodedURLString =
                        notification.targetURL.addingPercentEncoding(
                ...
                guard let targetURL = url else {
                    assertionFailure(
                        "Invalid target URL for creative instance id: \(notification.creativeInstanceID)"
                    )
                    return
                }
    ...
    override func viewDidLoad() {
        ...
        pasteGoAction = AccessibleAction(
            name: Strings.pasteAndGoTitle,
            handler: { () -> Bool in
        ...
        copyAddressAction = AccessibleAction(
            name: Strings.copyAddressTitle,
            handler: { () -> Bool in
                ...
                if !success {
                    DispatchQueue.main.async {
                        let alert = UIAlertController(
                            title: Strings.Sync.v2MigrationErrorTitle,
        ...
        if #available(iOS 14, *),
            !Preferences.DefaultBrowserIntro.defaultBrowserNotificationScheduled.value
        {
    ...
    private func migrateToChromiumBookmarks(_ completion: @escaping (_ success: Bool) -> Void) {
        let showInterstitialPage = { (url: URL?) -> Bool in
            ...
            return BookmarksInterstitialPageHandler.showBookmarksPage(
    ...
    fileprivate func setupTabs() {
        contentBlockListDeferred?.uponQueue(.main) { _ in
            ...
            if noTabsAdded {
                ...
                tabToSelect =
                    isPrivate
                    ? self.tabManager.addTab(isPrivate: true) : self.tabManager.restoreAllTabs()
            } else {
    ...
    func presentOnboardingIntro() {
        ...
        if Preferences.General.basicOnboardingCompleted.value == OnboardingState.completed.rawValue {
            ...
            if !BraveAds.isCurrentLocaleSupported()
                && Preferences.General.basicOnboardingProgress.value
                    == OnboardingProgress.rewards.rawValue
            {
                return
            }
        }
        ...
        let currentProgress =
            OnboardingProgress(rawValue: Preferences.General.basicOnboardingProgress.value) ?? .none
        ...
        if Preferences.General.basicOnboardingCompleted.value == OnboardingState.skipped.rawValue {
            ...
            guard
                let daysUntilNextPrompt = Preferences.General.basicOnboardingNextOnboardingPrompt
                    .value
            else {
                return
            }
            ...
            if daysUntilNextPrompt <= Date() && !isRewardsEnabled {
                guard
                    let onboarding = OnboardingNavigationController(
                        profile: profile,
                ...
                Preferences.General.basicOnboardingNextOnboardingPrompt.value = Date(
                    timeIntervalSinceNow: BrowserViewController.onboardingDaysInterval
                )
            }
        ...
        if (Preferences.General.basicOnboardingCompleted.value == OnboardingState.completed.rawValue)
            && (Preferences.General.basicOnboardingProgress.value
                == OnboardingProgress.searchEngine.rawValue)
        {

            guard !isRewardsEnabled,
                let onboarding = OnboardingNavigationController(
                    profile: profile,
                    onboardingType: .existingUserRewardsOff(currentProgress),
                    rewards: rewards,
        ...
        if (Preferences.General.basicOnboardingCompleted.value == OnboardingState.completed.rawValue)
            && (Preferences.General.basicOnboardingProgress.value
                == OnboardingProgress.none.rawValue)
        {
    ...
    private func presentVPNCallout() {
        ...
        let notEnoughAppLaunches =
            Preferences.VPN.appLaunchCountForVPNPopup.value < BraveVPN.appLaunchesToShowVPNPopup
    ...
    fileprivate func shouldShowWhatsNewTab() -> Bool {
        guard
            let latestMajorAppVersion = profile.prefs.stringForKey(LatestAppVersionProfileKey)?
    ...
    override func observeValue(
        ...
        guard let webView = object as? WKWebView else {
            log.error(
                "An object of type: \(String(describing: object)) is being observed instead of a WKWebView"
            )
        ...
        guard let tab = tabManager[webView] else {
            log.error(
                "WebView: \(webView) has been removed from TabManager but still has attached observers"
            )
            return
        }
        ...
        case .title:
            ...
            guard
                let title =
                    (webView.title?.isEmpty == true ? webView.url?.absoluteString : webView.title)
            else { break }
        ...
        case .canGoForward:
            guard tab === tabManager.selectedTab, let canGoForward = change?[.newKey] as? Bool
            else {
                break
            }
    ...
    fileprivate func runScriptsOnWebView(_ webView: WKWebView) {
        ...
        if NoImageModeHelper.isActivated {
            webView.evaluateSafeJavaScript(
                functionName: "__firefox__.NoImageMode.setEnabled",
                args: ["true"]
            )
        }
    }
    ...
    func switchToPrivacyMode(isPrivate: Bool) {
        let tabTrayController =
            self.tabTrayController
            ?? TabTrayController(tabManager: tabManager, profile: profile, tabTrayDelegate: self)
    ...
    func openURLInNewTab(_ url: URL?, isPrivate: Bool = false, isPrivileged: Bool) {
        ...
        if let url = url {
            request =
                isPrivileged ? PrivilegedRequest(url: url) as URLRequest : URLRequest(url: url)
        } else {
            request = nil
        }
    ...
    fileprivate func presentActivityViewController(
        ...
        if !url.isFileURL {
                    ...
                    let addToBraveToday = AddFeedToBraveTodayActivity { [weak self] in
                        ...
                        if #available(iOS 13.0, *) {
                            container.modalPresentationStyle =
                                idiom == .phone ? .pageSheet : .formSheet
                        } else {
                            container.modalPresentationStyle =
                                idiom == .phone ? .fullScreen : .formSheet
                        }
            ...
            #if compiler(>=5.3)
                if #available(iOS 14.0, *), let webView = tab?.webView,
                    tab?.temporaryDocument == nil
                {
                    let createPDFActivity = CreatePDFActivity(webView: webView) {
                        ...
                        let filename = webView.title?.components(separatedBy: validFilenameSet)
                            .joined()
            ...
            if let selectedTab = tabManager.selectedTab,
                selectedTab.mimeType == "application/xml"
                    || selectedTab.mimeType == "application/json",
        ...
        let controller = helper.createActivityViewController(items: activities) {
    ...
    func presentDuckDuckGoCallout(force: Bool = false) {
        ...
        if let presentedViewController = self.presentedViewController,
            presentedViewController.isKind(of: OnboardingNavigationController.self)
        {
            return
        }
        ...
        if !SearchEngines.shouldShowDuckDuckGoPromo
            || (Preferences.Popups.duckDuckGoPrivateSearch.value && !force)
        {
            return
        }

        // Do not show ddg popup if user already chose it for private browsing.
        if profile.searchEngines.defaultEngine(forType: .privateMode).shortName
            == OpenSearchEngine.EngineNames.duckDuckGo
        {
            return
        }
        ...
        let popup = AlertPopupView(
            imageView: UIImageView(image: #imageLiteral(resourceName: "duckduckgo")),
            title: Strings.DDGCalloutTitle,
            message: Strings.DDGCalloutMessage
        )
        ...
        popup.addButton(title: Strings.DDGCalloutEnable, type: .primary) { [weak self] in
            ...
            Preferences.Popups.duckDuckGoPrivateSearch.value = true
            self.profile.searchEngines.updateDefaultEngine(
                OpenSearchEngine.EngineNames.duckDuckGo,
                forType: .privateMode
            )
    ...
    private func presentSettingsNavigation(
...
extension BrowserViewController: SettingsDelegate {
    ...
    func settingsDidFinish(_ settingsViewController: SettingsViewController) {
        settingsViewController.dismiss(
            animated: true,
            completion: {
...
extension BrowserViewController: TopToolbarDelegate {
    ...
    func topToolbarLocationAccessibilityActions(_ topToolbar: TopToolbarView)
...
extension BrowserViewController: ToolbarDelegate {
    ...
    private func addTabAlertActions() -> [UIAlertAction] {
        ...
        if !PrivateBrowsingManager.shared.isPrivateBrowsing {
            let newPrivateTabAction = UIAlertAction(
...
extension BrowserViewController: TabDelegate {
    ...
    func tab(_ tab: Tab, didCreateWebView webView: WKWebView) {
        ...
        tab.addContentScript(
            tab.contentBlocker,
            name: ContentBlockerHelper.name(),
            sandboxed: false
        )
        ...
        tab.addContentScript(
            FingerprintingProtection(tab: tab),
            name: FingerprintingProtection.name(),
            sandboxed: false
        )
        ...
        if YubiKitDeviceCapabilities.supportsMFIAccessoryKey {
            tab.addContentScript(
                U2FExtensions(tab: tab),
                name: U2FExtensions.name(),
                sandboxed: false
            )
        }

        tab.addContentScript(
            ResourceDownloadManager(tab: tab),
            name: ResourceDownloadManager.name(),
            sandboxed: false
        )

        tab.addContentScript(
            WindowRenderHelperScript(tab: tab),
            name: WindowRenderHelperScript.name(),
            sandboxed: false
        )

        tab.addContentScript(
            RewardsReporting(rewards: rewards, tab: tab),
            name: RewardsReporting.name()
        )
        tab.addContentScript(
            AdsMediaReporting(rewards: rewards, tab: tab),
...
extension BrowserViewController: TabManagerDelegate {
    ...
    func tabManager(_ tabManager: TabManager, didRemoveTab tab: Tab) {
        ...
        if let url = tab.url, !url.isAboutURL && !tab.isPrivate {
            profile.recentlyClosedTabs.addTab(
                url as URL,
                title: tab.title,
...
extension BrowserViewController: WKUIDelegate {
    func webView(
        ...
        guard navigationAction.isAllowed, shouldRequestBeOpenedAsPopup(navigationAction.request)
        else {
        ...
        case .edited:
            ...
            let editPopup =
                ...
                .userTextInputAlert(
                    ...
                    if let cTitle = callbackTitle, !cTitle.isEmpty, let cUrl = callbackUrl,
                        !cUrl.isEmpty
                    {
...
extension BrowserViewController: PreferencesObserver {
    func preferencesDidChange(for key: String) {
        ...
        default:
            log.debug(
                "Received a preference change for an unknown key: \(key) on \(type(of: self))"
            )
            break
        }
    }
