...
extension Int32 {
    fileprivate var loggerLevel: XCGLogger.Level {
        switch self {
        case 0: return .error
        case 1: return .info
...
extension BrowserViewController {
    func updateRewardsButtonState() {
        ...
        self.topToolbar.locationView.rewardsButton.isHidden =
            Preferences.Rewards.hideRewardsIcon.value
            || PrivateBrowsingManager.shared.isPrivateBrowsing
        self.topToolbar.locationView.rewardsButton.iconState =
    ...
    func showBraveRewardsPanel() {
        ...
        let popover = PopoverController(
            contentController: braveRewardsPanel,
            contentSizeBehavior: .autoLayout
        )
        ...
        popover.popoverDidDismiss = { [weak self] _ in
            ...
            if let tabId = self.tabManager.selectedTab?.rewardsId,
                self.rewards.ledger.selectedTabId == 0
            {
    ...
    func authorizeUpholdWallet(from tab: Tab, queryItems items: [String: String]) {
        rewards.ledger.authorizeExternalWallet(
            ...
            case .batNotAllowed:
                ...
                popup.addButton(
                    title: Strings.userWalletBATNotAllowedLearnMore,
                    type: .link,
                    fontSize: 14.0
    ...
    static func migrateAdsConfirmations(for configruation: BraveRewardsConfiguration) {
        ...
        do {
            ...
            try confirmations.write(
                toFile: adsConfirmations.path,
                atomically: true,
                encoding: .utf8
            )
        } catch {
...
extension Tab {
    func reportPageLoad(to rewards: BraveRewards) {
        ...
        webView.evaluateSafeJavaScript(functionName: "document.documentElement.outerHTML.toString") {
            html,
            _ in
