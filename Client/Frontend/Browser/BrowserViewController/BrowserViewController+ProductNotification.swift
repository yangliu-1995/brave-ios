...
extension BrowserViewController {
    ...
    enum BenchmarkTrackerCountTier: Int, Equatable, CaseIterable {
        ...
        var title: String {
            ...
            case .newbieExclusiveTier, .casualExclusiveTier, .regularExclusiveTier,
                .expertExclusiveTier:
    ...
    private func presentEducationalProductNotifications() {
        ...
        guard let selectedTab = tabManager.selectedTab,
            ...
            tabManager.selectedTab?.url?.isAboutHomeURL == false
        else {
            return
        }
        ...
        if Locale.current.regionCode == "JP" {
            let numOfTrackerAds =
                ...
                + BraveGlobalShieldStats.shared.trackingProtection
            ...
            if !existingTierList.isEmpty {
                ...
                if numOfTrackerAds > firstExistingTier.value {
                    notifyTrackerAdsCount(
                        ...
                        description: firstExistingTier.title,
                        theme: Theme.of(selectedTab)
                    )
                }
            }
        }
    ...
    private func notifyPrivacyProtectBlock(theme: Theme) {
        let shareTrackersViewController = ShareTrackersController(
            theme: theme,
            trackingType: .trackerAdCountBlock(count: benchmarkNumberOfTrackers)
        )
        dismiss(animated: true)
        showBenchmarkNotificationPopover(controller: shareTrackersViewController)
    }
    ...
    private func notifyHttpsUpgrade(theme: Theme) {
        let shareTrackersViewController = ShareTrackersController(
            theme: theme,
            trackingType: .encryptedConnectionWarning
        )
        dismiss(animated: true)
        showBenchmarkNotificationPopover(controller: shareTrackersViewController)
    }
    ...
    private func notifyTrackerAdsCount(_ count: Int, description: String, theme: Theme) {
        let shareTrackersViewController = ShareTrackersController(
            theme: theme,
            trackingType: .trackerCountShare(count: count, description: description)
        )
        dismiss(animated: true)
