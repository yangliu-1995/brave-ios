...
extension BrowserViewController {
    fileprivate func handleExternalURL(
        _ url: URL,
        ...
        popup.addButton(title: Strings.openExternalAppURLDontAllow, fontSize: 16) {
            () -> PopupViewDismissType in
            return .flyDown
        }
        popup.addButton(title: Strings.openExternalAppURLAllow, type: .primary, fontSize: 16) {
            () -> PopupViewDismissType in
            UIApplication.shared.open(
                url,
                options: [:],
                completionHandler: openedURLCompletionHandler
            )
            return .flyDown
        }
...
extension BrowserViewController: WKNavigationDelegate {
    ...
    func webView(
        ...
        if let customHeader = UserReferralProgram.shouldAddCustomHeader(
            for: navigationAction.request
        ) {
            decisionHandler(.cancel)
            var newRequest = navigationAction.request
            UrpLog.log(
                "Adding custom header: [\(customHeader.field): \(customHeader.value)] for domain: \(newRequest.url?.absoluteString ?? "404")"
            )
        ...
        if let mainDocURL = navigationAction.request.mainDocumentURL,
            let universalLink = UniversalLinkManager.universalLinkType(
                for: mainDocURL,
                checkPath: true
            )
        {
        ...
        if ["http", "https", "data", "blob", "file"].contains(url.scheme) {
            ...
            if let urlHost = url.normalizedHost() {
                    ...
                    url.scheme == "http"
                {
                    ...
                    if domainForShields.isShieldExpected(.HTTPSE, considerAllShieldsOption: true)
                        && HttpsEverywhereStats.shared.shouldUpgrade(url)
                    {
                    ...
                    || navigationAction.targetFrame?.isMainFrame == true
            {
                ...
                if let tab = tabManager[webView] {
                    tab.userScriptManager?.isFingerprintingProtectionEnabled =
                        domainForShields.isShieldExpected(
                            .FpProtection,
            ...
            if let tab = tabManager[webView] {
                tab.userScriptManager?.isCookieBlockingEnabled =
            ...
            if let newHost: String = url.host, let oldHost: String = webView.url?.host,
                newHost != oldHost
            {
        ...
        if let url = responseURL, let urlHost = responseURL?.normalizedHost() {
            ...
            if url.scheme == "https", let _ = pendingHTTPUpgrades.removeValue(forKey: urlHost) {
                ...
                if let stats = self.tabManager[webView]?.contentBlocker.stats {
                    self.tabManager[webView]?.contentBlocker.stats = stats.create(
                        byAddingListItem: .https
                    )
                }
            }
        }
