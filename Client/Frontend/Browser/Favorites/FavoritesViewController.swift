...
extension FavoritesViewController: KeyboardHelperDelegate {
    func updateKeyboardInset(_ state: KeyboardState, animated: Bool = true) {
        ...
        UIView.animate(
            withDuration: animated ? state.animationDuration : 0.0,
            animations: {
                if animated {
                ...
                self.collectionView.scrollIndicatorInsets = self.collectionView
                    .scrollIndicatorInsets.with {
                        $0.bottom = keyboardHeight
                    }
            }
        )
        ...
        -> UICollectionViewCell
    {
        ...
        if let url = fav.url?.asURL {
            ...
            cell.imageView.loadFavicon(
                siteURL: url,
                domain: domain,
                monogramFallbackCharacter: fav.title?.first
            )
        }
