...
class NTPDownloader {
    ...
    private func getNTPResource(
        for type: ResourceType,
        _ completion: @escaping (NTPThemeable?) -> Void
    ) {
            ...
            return baseTime
                * Double.random(
                    in: ClosedRange<Double>(
                        uncheckedBounds: (lower: minVariance, upper: maxVariance)
                    )
                )
        }()
    ...
    private func scheduleObservers(relativeTime: TimeInterval) {
        ...
        self.removeObservers()
        self.timer = Timer.scheduledTimer(withTimeInterval: relativeTime, repeats: true) {
            [weak self] _ in
    ...
    private func loadNTPResource(for type: ResourceType) -> NTPThemeable? {
        do {
            ...
            case .sponsor:
                ...
                let wallpapers = mapNTPBackgroundsToFullPath(
                    sponsor.wallpapers,
                    basePath: downloadsFolderURL
                )
            ...
            case .superReferral(let code):
                ...
                return CustomTheme(
                    themeName: customTheme.themeName,
                    wallpapers: wallpapers,
                    logo: logo,
                    topSites: customTheme.topSites,
                    refCode: code
                )
            }
        } catch {
    ...
    private func mapNTPBackgroundsToFullPath(
        _ backgrounds: [NTPBackground],
        basePath: URL
    ) -> [NTPBackground] {
        backgrounds.map {
            NTPBackground(
    ...
    private func downloadMetadata(
        type: ResourceType,
        _ completion: @escaping (URL?, CacheResponse?, NTPError?) -> Void
    ) {
        self.download(type: type, path: type.resourceName, etag: self.getETag(type: type)) {
            [weak self] data, cacheInfo, error in
            guard let self = self else { return }
            ...
            guard let data = data else {
                return completion(
                    nil,
                    nil,
                    .metadataError("Invalid \(type.resourceName) for NTP Download")
                )
            }
    ...
    private func download(
        ...
        path: String?,
        etag: String?,
        _ completion: @escaping (Data?, CacheResponse?, Error?) -> Void
    ) {
        ...
        URLSession(configuration: .ephemeral).dataRequest(with: request) {
    ...
    private func unpackMetadata(
        type: ResourceType,
        data: Data,
        _ completion: @escaping (URL?, NTPError?) -> Void
    ) {
                ...
                for topSite in topSites {
                    ...
                    self.download(type: type, path: topSite.iconUrl, etag: nil) { data, _, err in
                        ...
                        do {
                            ...
                            let topSiteBackgroundColorURL = saveLocation.appendingPathComponent(
                                topSiteBackgroundColorFileName
                            )
