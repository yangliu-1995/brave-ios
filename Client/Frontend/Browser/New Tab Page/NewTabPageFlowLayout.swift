...
class NewTabPageFlowLayout: UICollectionViewFlowLayout {
    ...
    override func prepare() {
            ...
            )
        {
            ...
            if let lastItemAttribute = super.layoutAttributesForItem(
                at: IndexPath(item: numberOfItems - 1, section: braveTodaySection)
            ) {
                if lastItemAttribute.frame.maxY - attribute.frame.minY
                ...
                if attribute.indexPath.item == 0 {
                    ...
                    if let previousItemAttribute = layoutAttributesForItem(
                        ...
                        attribute.frame.origin.x =
                            previousItemAttribute.frame.maxX + minimumInteritemSpacing
                    }
                }
            }
        }
    ...
    override func targetContentOffset(
        ...
        guard let braveTodaySection = braveTodaySection,
            collectionView?.numberOfItems(inSection: braveTodaySection) != 0,
            let item = layoutAttributesForItem(at: IndexPath(item: 0, section: braveTodaySection))
        else {
    ...
    override func shouldInvalidateLayout(
        ...
        {
            return preferredAttributes.size.height.rounded()
                != originalAttributes.size.height.rounded()
        }
