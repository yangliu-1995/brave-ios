...
class OnboardingNavigationController: UINavigationController {
            ...
            if BraveRewards.isAvailable {
                ...
                case .newUser(let progress):
                    ...
                    var newUserScreens: [Screens] = [
                        .searchEnginePicker, .shieldsInfo, .rewardsAgreement,
                    ]
    ...
    fileprivate enum Screens {
        ...
        func viewController(with profile: Profile, rewards: BraveRewards?, theme: Theme)
            -> OnboardingViewController
        {
            switch self {
            case .privacyConsent:
                return OnboardingPrivacyConsentViewController(
                    profile: profile,
                    rewards: rewards,
                    theme: theme
                )
            case .searchEnginePicker:
                return OnboardingSearchEnginesViewController(
                    profile: profile,
                    rewards: rewards,
                    theme: theme
                )
            case .shieldsInfo:
                return OnboardingShieldsViewController(
                    profile: profile,
                    rewards: rewards,
                    theme: theme
                )
            case .rewardsAgreement:
                return OnboardingRewardsAgreementViewController(
                    profile: profile,
                    rewards: rewards,
                    theme: theme
                )
    ...
    convenience init?(
        ...
        let firstViewController = firstScreen.viewController(
            with: profile,
            rewards: rewards,
            theme: theme
        )
...
extension OnboardingNavigationController: Onboardable {
    ...
    func presentNextScreen(current: OnboardingViewController) {
        ...
        guard let nextIndex = index?.advanced(by: 1),
            let nextScreen = allScreens[safe: nextIndex]?.viewController(
    ...
    func presentPreviousScreen(current: OnboardingViewController) {
        ...
        guard let previousIndex = index?.advanced(by: -1),
            let previousScreen = viewControllers[previousIndex] as? OnboardingViewController
        else {
...
class CustomAnimator: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        let fDetailsContent = fromView.subview(
            with: OnboardingViewAnimationID.detailsContent.rawValue
        )
        ...
        let tDetailsContent = toView.subview(
            with: OnboardingViewAnimationID.detailsContent.rawValue
        )
        ...
        fDetailsFrame.origin.y =
            (container.frame.height - container.frame.origin.y) - fDetailsFrame.height
        fDetailsFrame = fDetailsFrame.offsetBy(dx: 0.0, dy: -inset)

        var tDetailsFrame = (tDetails?.bounds ?? .zero)
        tDetailsFrame.origin.y =
            (container.frame.height - container.frame.origin.y) - tDetailsFrame.height
        tDetailsFrame = tDetailsFrame.offsetBy(dx: 0.0, dy: -inset)

        // Pause animations..
        if !shouldFadeGraphics, let fAnimation = fBackground as? AnimationView,
            let tAnimation = tBackground as? AnimationView
        {
        ...
        DispatchQueue.main.asyncAfter(
            deadline: .now() + self.transitionDuration(using: transitionContext)
        ) {
