...
extension OnboardingPrivacyConsentViewController {
    ...
    class View: UIView {
        ...
        let yesConsentButton = CommonViews.primaryButton(text: Strings.OBPrivacyConsentYesButton)
            .then {
                $0.accessibilityIdentifier = "OnboardingPrivacyConsentViewController.YesButton"
                $0.titleLabel?.adjustsFontSizeToFitWidth = true
            }

        let noConsentButton = CommonViews.secondaryButton(text: Strings.OBPrivacyConsentNoButton)
            .then {
        ...
        private let braveLogo = UIImageView(
            image: #imageLiteral(resourceName: "browser_lock_popup")
        )
        .then {
        ...
        init(theme: Theme) {
            ...
            mainStackView.addStackViewItems(
                ...
                .view(
                    UIStackView(arrangedSubviews: [yesConsentButton, noConsentButton]).then {
                        $0.axis = .vertical
                        $0.spacing = 8
                    }
                )
