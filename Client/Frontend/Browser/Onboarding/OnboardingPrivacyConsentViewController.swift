...
class OnboardingPrivacyConsentViewController: OnboardingViewController {
    ...
    override func viewDidLoad() {
        ...
        contentView.yesConsentButton.addTarget(
            self,
            action: #selector(yesConsentTapped),
            for: .touchUpInside
        )
        contentView.noConsentButton.addTarget(
            self,
            action: #selector(noConsentTaapped),
            for: .touchUpInside
        )
    }
    ...
    private func presentNextScreen(withPrivacyConsent: Bool) {
        Preferences.General.basicOnboardingProgress.value =
            OnboardingProgress.privacyConsent.rawValue
