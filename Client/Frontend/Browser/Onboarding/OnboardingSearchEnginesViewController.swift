...
class OnboardingSearchEnginesViewController: OnboardingViewController {
    ...
    override func viewDidLoad() {
        ...
        contentView.searchEnginesTable.register(
            SearchEngineCell.self,
            forCellReuseIdentifier: String(describing: SearchEngineCell.self)
        )
        ...
        DispatchQueue.main.async {
            for searchEngine in self.availableEngines.enumerated() {
                if searchEngine.element.shortName == self.searchEngines.defaultEngine().shortName {
                    ...
                    self.contentView.searchEnginesTable.selectRow(
                        at: indexPath,
                        animated: false,
                        scrollPosition: .none
                    )
    ...
    @objc override func continueTapped() {
        guard let selectedRow = contentView.searchEnginesTable.indexPathForSelectedRow?.section,
            let selectedEngine = availableEngines[safe: selectedRow]?.shortName
        else {
            return
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard
            let cell = tableView.dequeueReusableCell(
                withIdentifier: String(describing: SearchEngineCell.self),
                for: indexPath
            )
                as? SearchEngineCell
        else {
