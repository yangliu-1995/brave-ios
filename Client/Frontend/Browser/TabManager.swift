...
class TabManager: NSObject {
    ...
    private class func getNewConfiguration() -> WKWebViewConfiguration {
        ...
        UserReferralProgram.shared?.insertCookies(
            intoStore: configuration.websiteDataStore.httpCookieStore
        )
    ...
    func selectTab(_ tab: Tab?, previous: Tab? = nil) {
        ...
        guard tab === selectedTab else {
            log.error(
                "Expected tab (\(String(describing: tab?.url))) is not selected. Selected index: \(selectedIndex)"
            )
            return
        }
        ...
        delegates.forEach {
            $0.get()?.tabManager(self, didSelectedTabChange: tab, previous: previous)
        }
        if let tab = previous {
        ...
        guard let appDelegate = UIApplication.shared.delegate as? AppDelegate,
            let newSelectedTab = tab, let previousTab = previous,
            let newTabUrl = newSelectedTab.url,
        ...
        if !PrivateBrowsingManager.shared.isPrivateBrowsing {
            ...
            if faviconURL == nil && !newTabUrl.isLocal {
                rewardsLog.warning(
                    "No favicon found in \(newSelectedTab) to report to rewards panel"
                )
            }
            rewards.reportTabUpdated(
                Int(newSelectedTab.rewardsId),
                url: newTabUrl,
    ...
    func moveTab(_ tab: Tab, toIndex visibleToIndex: Int) {
        ...
        guard let fromIndex = allTabs.firstIndex(of: tab),
            let toIndex = allTabs.firstIndex(of: toTab)
        else {
            return
        }
    ...
    func savedTabData(tab: Tab) -> SavedTab? {
        ...
        guard let urlString = tab.url?.absoluteString, !urlString.contains("localhost") else {
            return nil
        }
        ...
        if let numberOfTabs = tempTabs?.count, numberOfTabs > 0 {
            toast = ButtonToast(
                labelText: String.localizedStringWithFormat(
                    Strings.tabsDeleteAllUndoTitle,
                    numberOfTabs
                ),
    ...
    static fileprivate func tabsStateArchivePath() -> String {
        guard
            ...
            return URL(fileURLWithPath: documentsPath).appendingPathComponent("tabsState.archive")
                .path
        }
    ...
    static fileprivate func migrateTabsStateArchive() {
        ...
        let newPath = URL(fileURLWithPath: profilePath).appendingPathComponent("tabsState.archive")
            .path
        ...
        do {
            ...
            log.error(
                "Unable to move tabsState.archive to shared container: \(error.localizedDescription)"
            )
        }
    }
    ...
    private func preserveScreenshots() {
        ...
        for tab in allTabs {
            guard let screenshot = tab.screenshot, let screenshotUUID = tab.screenshotUUID else {
                continue
            }
    ...
    fileprivate lazy var restoreTabsInternal: () -> Tab? = {
        ...
        for savedTab in savedTabs {
            ...
            if let url = URL(string: urlString),
                let faviconURL = Domain.getOrCreate(forUrl: url, persistent: !isPrivateBrowsing)
                    .favicon?
                    .url
            {
    ...
    func restoreTab(_ tab: Tab) {
        ...
        if let history = savedTab.urlHistorySnapshot as? [String], let tabUUID = savedTab.syncUUID,
            let url = savedTab.url
        {
            let data = SavedTab(
                ...
                url: url,
                isSelected: savedTab.isSelected,
                order: savedTab.order,
                screenshot: nil,
                history: history,
                historyIndex: savedTab.urlHistoryCurrentIndex
            )
...
extension TabManager: WKNavigationDelegate {
    ...
    func webView(_ webView: WKWebView, didCommit navigation: WKNavigation!) {
        ...
        if !tab.contentBlocker.isEnabled {
            webView.evaluateSafeJavaScript(
                functionName: "window.__firefox__.TrackingProtectionStats.setEnabled",
                args: [false, UserScriptManager.securityToken],
                sandboxed: false
            )
        }
    }
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        ...
        if let tab = tabForWebView(webView), !tab.isPrivate, let url = webView.url,
...
extension TabManager: PreferencesObserver {
    func preferencesDidChange(for key: String) {
        ...
        case Preferences.General.blockPopups.key:
            ...
            allTabs.forEach {
                $0.webView?.configuration.preferences.javaScriptCanOpenWindowsAutomatically =
                    allowPopups
            }
