...
class TabsBarViewController: UIViewController {
    ...
    @objc func handleLongGesture(gesture: UILongPressGestureRecognizer) {
        ...
        case .began:
            guard
                let selectedIndexPath = self.collectionView.indexPathForItem(
...
extension TabsBarViewController: UICollectionViewDataSource {
        ...
        -> UICollectionViewCell
    {
        ...
        cell.closeTabCallback = { [weak self] tab in
            guard let strongSelf = self, let tabManager = strongSelf.tabManager,
                let previousIndex = strongSelf.tabList.index(of: tab)
            else { return }
...
extension TabsBarViewController: Themeable {
    func applyTheme(_ theme: Theme) {
        ...
        bottomLine.backgroundColor = theme.colors.border.withAlphaComponent(
