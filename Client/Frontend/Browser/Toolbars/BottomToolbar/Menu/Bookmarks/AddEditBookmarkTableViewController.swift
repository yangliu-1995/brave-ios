...
class AddEditBookmarkTableViewController: UITableViewController {
    ...
    private var rootLevelFolderCell: IndentedImageTableViewCell {
        let cell = IndentedImageTableViewCell(image: #imageLiteral(resourceName: "menu_bookmarks"))
            .then {
        ...
        -> UITableViewCell
    {
        ...
        case .currentSelection:
            ...
            case .folder(let folder):
                let cell = IndentedImageTableViewCell(
                    image: #imageLiteral(resourceName: "menu_folder")
                )
        ...
        case .folderHierarchy:
            ...
            if let folder = saveLocation.getFolder,
                folder.objectID == indentedFolder.folder.objectID
            {
                cell.accessoryType = .checkmark
            } else if case .rootLevel = saveLocation,
                indentedFolder.folder.objectID == self.rootFolderId
            {
                cell.accessoryType = .checkmark
            }
...
extension AddEditBookmarkTableViewController: BookmarkDetailsViewDelegate {
    func correctValues(validationPassed: Bool) {
        ...
        navigationController?.navigationBar.topItem?.rightBarButtonItem?.isEnabled =
            validationPassed
    }
}
