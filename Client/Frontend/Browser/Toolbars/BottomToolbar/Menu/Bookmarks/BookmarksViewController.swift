...
class BookmarksViewController: SiteTableViewController, ToolbarUrlActionsProtocol {
    ...
    private func setUpToolbar() {
        ...
        let items = (leftToolbarItems + [flexibleSpace, rightItem, .fixedSpace(5)]).compactMap {
            $0
        }
    ...
    private func showLastVisitedFolder() {
        DispatchQueue.main.async {
            ...
            if index <= 0 && self.currentFolder != nil {
                ...
                nextController.navigationItem.setRightBarButton(
            ...
            if !editing {
                cell.addGestureRecognizer(
                    UILongPressGestureRecognizer(
                        target: self,
                        action: #selector(longPressedCell(_:))
                    )
                )
            }
        }
    ...
    @objc private func importExportAction(_ sender: UIBarButtonItem) {
        ...
        let importAction = UIAlertAction(title: Strings.bookmarksImportAction, style: .default) {
            ...
            let vc = UIDocumentPickerViewController(
                documentTypes: [String(kUTTypeHTML)],
                in: .import
            )
            vc.delegate = self
    ...
    fileprivate func configureCell(_ cell: BookmarkTableViewCell, atIndexPath indexPath: IndexPath) {
        ...
        func configCell(image: UIImage? = nil, icon: FaviconMO? = nil) {
            ...
            if let image = image {
                ...
                let setFavIcon = { (cell: UITableViewCell, item: Bookmarkv2) in
                    ...
                    if let icon = item.icon {
                        ...
                        cell.imageView?.loadFavicon(
                            for: url,
                            domain: domain,
                            fallbackMonogramCharacter: item.title?.first,
                            cachedOnly: true
                        )
                    } else {
                        cell.imageView?.clearMonogramFavicon()
                ...
                if (item.icon == nil && (item.isFavIconLoading || item.isFavIconLoaded))
                    || item.icon != nil
                {
                    setFavIcon(cell, item)
                } else if let domain = item.domain, let url = domain.url?.asURL {
    ...
    @objc private func longPressedCell(_ gesture: UILongPressGestureRecognizer) {
        guard gesture.state == .began,
            ...
            let bookmark = bookmarksFRC?.object(at: indexPath)
        else {
            return
        }

        presentLongPressActions(
            gesture,
        ...
        let deleteAction = UITableViewRowAction(
            ...
            handler: { action, indexPath in
                ...
                if let children = item.children, !children.isEmpty {
                    ...
                    alert.addAction(
                        UIAlertAction(title: Strings.yesDeleteButtonTitle, style: .destructive) {
                            _ in
                            item.delete()
                        }
                    )
