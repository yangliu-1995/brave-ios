...
class DownloadsPanel: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        tableView.register(
            TwoLineTableViewCell.self,
            forCellReuseIdentifier: "TwoLineTableViewCell"
        )
    ...
    func reloadData() {
        ...
        for downloadedFile in downloadedFiles {
            groupedDownloadedFiles.add(
                downloadedFile,
                timestamp: downloadedFile.lastModified.timeIntervalSince1970
            )
        }
    ...
    private func fetchData() -> [DownloadedFile] {
        ...
        do {
            ...
            for file in files {
                let attributes =
                    try FileManager.default.attributesOfItem(atPath: file.path) as NSDictionary
    ...
    private func roundRectImageWithLabel(
        ...
        attributedString.draw(
            at: CGPoint(
                x: (width - stringWidth) / 2 + strokeWidth,
                y: (height - stringHeight) / 2 + strokeWidth
            )
        )
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard
            let cell = tableView.dequeueReusableCell(
                withIdentifier: "TwoLineTableViewCell",
                for: indexPath
            ) as? TwoLineTableViewCell
        else {
    ...
    func configureDownloadedFile(_ cell: UITableViewCell, for indexPath: IndexPath)
        -> UITableViewCell
    {
