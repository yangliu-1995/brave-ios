...
extension ToolbarUrlActionsProtocol {
    func presentLongPressActions(
        ...
        alert.title = urlString?.replacingOccurrences(of: "mailto:", with: "").ellipsize(
            maxLength: 120
        )
        ...
        if let popoverPresentationController = alert.popoverPresentationController {
            ...
            popoverPresentationController.sourceRect = CGRect(
                origin: gesture.location(in: view),
    ...
    private func actions(forUrl url: URL?, currentTabIsPrivate: Bool) -> [UIAlertAction] {
        ...
        return [newTabAction, newPrivateTabAction, copyAction, shareAction].compactMap { $0 }.map {
            action in
