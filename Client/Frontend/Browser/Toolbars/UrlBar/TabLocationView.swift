...
protocol TabLocationViewDelegate {
    ...
    @discardableResult func tabLocationViewDidLongPressReaderMode(
        _ tabLocationView: TabLocationView
    )
        -> Bool
...
class TabLocationView: UIView {
        ...
        didSet {
            if loading {
                reloadButton.setImage(
                    #imageLiteral(resourceName: "nav-stop").template,
                    for: .normal
                )
                reloadButton.accessibilityLabel = Strings.tabToolbarStopButtonAccessibilityLabel
            } else {
                reloadButton.setImage(
                    #imageLiteral(resourceName: "nav-refresh").template,
                    for: .normal
                )
    ...
    lazy var placeholder: NSAttributedString = {
        return NSAttributedString(
            ...
            attributes: [NSAttributedString.Key.foregroundColor: UIColor.Photon.grey40]
        )
    }()
    ...
    lazy var urlTextField: UITextField = {
        ...
        urlTextField.setContentCompressionResistancePriority(
    ...
    fileprivate lazy var lockImageView: UIImageView = {
        let lockImageView = UIImageView(
            image: #imageLiteral(resourceName: "lock_verified").template
        )
        lockImageView.isHidden = true
        lockImageView.tintColor = #colorLiteral(
            red: 0.3764705882,
            green: 0.3843137255,
            blue: 0.4,
            alpha: 1
    ...
    fileprivate lazy var readerModeButton: ReaderModeButton = {
        ...
        readerModeButton.addTarget(
            self,
            action: #selector(tapReaderModeButton),
            for: .touchUpInside
        )
        readerModeButton.addGestureRecognizer(
            UILongPressGestureRecognizer(target: self, action: #selector(longPressReaderModeButton))
        )
    ...
    lazy var separatorLine: UIView = CustomSeparatorView(
        lineSize: .init(width: 1, height: 26),
        cornerRadius: 2
    )
    ...
    override init(frame: CGRect) {
        ...
        longPressRecognizer = UILongPressGestureRecognizer(
            target: self,
            action: #selector(longPressLocation)
        )
        ...
        optionSubviews.forEach {
            ($0 as? CustomSeparatorView)?.layoutMargins = UIEdgeInsets(
                top: 0,
                left: 2,
                bottom: 0,
                right: 2
        ...
        contentView.layoutMargins = UIEdgeInsets(
            top: 0,
            left: TabLocationViewUX.spacing,
            bottom: 0,
            right: 0
        )
    ...
    override var accessibilityElements: [Any]? {
        get {
            return [lockImageView, urlTextField, readerModeButton, reloadButton, shieldsButton]
                .filter {
                    !$0.isHidden
                }
        }
...
extension TabLocationView: Themeable {
    ...
    func applyTheme(_ theme: Theme) {
        ...
        backgroundColor = theme.colors.addressBar.withAlphaComponent(
            theme.colors.transparencies.addressBarAlpha
        )
        ...
        separatorLine.backgroundColor = theme.colors.border.withAlphaComponent(
            theme.colors.transparencies.borderAlpha
        )
    }
}
...
class DisplayTextField: UITextField {
    ...
    override func textRect(forBounds bounds: CGRect) -> CGRect {
        ...
        if let size: CGSize = (self.hostString as NSString?)?.size(withAttributes: [
            .font: self.font!
        ]) {
