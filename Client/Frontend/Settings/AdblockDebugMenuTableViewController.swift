...
class AdblockDebugMenuTableViewController: TableViewController {
    ...
    private var fetchSection: Section {
        var section = Section(
            footer:
                "Last time we pinged the server for new data. If adblock list hasn't changed `Last Time Updated` section does not update."
        )
    ...
    private func bundledListsSection(names: [String]) -> Section {
        ...
        names.forEach {
            if let bundlePath = Bundle.main.path(forResource: $0, ofType: "json") {
                ...
                rows.append(
                    .init(text: text, detailText: hashText, cellClass: ShrinkingSubtitleCell.self)
                )
            }
    ...
    private func downloadedListsSection(names: [String]) -> Section {
        ...
        func getEtag(name: String) -> String? {
            guard let folderUrl = fm.getOrCreateFolder(name: AdblockResourceDownloader.folderName)
            else {
                return nil
            }
        ...
        names.forEach {
                ...
                ) as? [[String: Any]]
            {
                ...
                rows.append(
                    .init(text: text, detailText: detail, cellClass: ShrinkingSubtitleCell.self)
                )
            }
