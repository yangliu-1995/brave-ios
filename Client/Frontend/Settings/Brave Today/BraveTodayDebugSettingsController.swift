        ...
        dataSource.sections = [
            .init(
                rows: [
                    Row(
                        text: "Environment",
                        detailText: feedDataSource.environment.name,
                        selection: { [unowned self] in
                            let picker = TodayEnvironmentPicker(
                                selectedEnvironment: feedDataSource.environment
                            ) {
                ...
                footer: .title(
                    "Changing the environment will purge all cached resources immediately."
                )
            ),
            .init(
                rows: [
                    Row(
                        text: "State",
                        detailText: feedDataSource.description(of: feedDataSource.state)
                    )
...
private class TodayEnvironmentPicker: TableViewController {
    ...
    override func viewDidLoad() {
        ...
        dataSource.sections = [
            .init(
                rows: FeedDataSource.Environment.allCases.map { environment in
                    Row(
