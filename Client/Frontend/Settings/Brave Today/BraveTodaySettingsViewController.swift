        ...
        dataSource.sections = [
            ...
            .init(
                ...
                rows: feedDataSource.rssFeedLocations.map { location in
                    ...
                    return Row(
                        ...
                        accessory: .switchToggle(
                            ...
                            { [unowned self] newValue in
                                self.feedDataSource.toggleRSSFeedEnabled(
                                    location,
                                    enabled: newValue
                                )
                            }
                        ...
                        editActions: [
                            .init(
                                title: Strings.BraveToday.deleteUserSourceTitle,
                                style: .destructive,
                                selection: { [unowned self] indexPath in
                                    guard
                                        let location = feedDataSource.rssFeedLocations[
                                            safe: indexPath.row
                                        ]
                                    else {
                                        return
                    ...
                    Row(
                        text: Strings.BraveToday.addSource,
                        selection: { [unowned self] in
                            let controller = BraveTodayAddSourceViewController(
        ...
        if !feedDataSource.sources.isEmpty {
            dataSource.sections.append(
                .init(
                    ...
                    rows: [
                        Row(
                            text: Strings.BraveToday.allSources,
                            selection: { [unowned self] in
                                let controller = FeedSourceListViewController(
                                    dataSource: self.feedDataSource,
                                    category: nil
                                )
            ...
            dataSource.sections.append(
                .init(rows: [
                    Row(
                        text: Strings.BraveToday.resetSourceSettingsButtonTitle,
                        selection: { [unowned self] in
