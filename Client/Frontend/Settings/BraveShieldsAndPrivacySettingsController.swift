    ...
    private lazy var shieldsSection: Section = {
        var shields = Section(
            ...
            rows: [
                ...
                .boolRow(
                    title: Strings.blockPhishingAndMalware,
                    option: Preferences.Shields.blockPhishingAndMalware
                ),
                .boolRow(
                ...
                .boolRow(
                    ...
                    detailText: Strings.blockCookiesDescription,
                    ...
                    onValueChange: { [unowned self] in
                        func toggleCookieSetting(with status: Bool) {
                            ...
                            let completionBlock: (Bool) -> Void = { _ in
                                ...
                                if success {
                                    ...
                                    alert.addAction(
                                        UIAlertAction(title: Strings.OKString, style: .default)
                                    )
                        ...
                        if $0 {
                            ...
                            let okAction = UIAlertAction(
                                title: Strings.blockAllCookiesAction,
                                style: .destructive,
                                handler: { (action) in
        ...
        if let locale = Locale.current.languageCode,
            let _ = ContentBlockerRegion.with(localeCode: locale)
        {
            shields.rows.append(
                .boolRow(
                    title: Strings.useRegionalAdblock,
                    option: Preferences.Shields.useRegionAdBlock
                )
            )
        }
        return shields
    ...
    private lazy var clearPrivateDataSection: Section = {
        return Section(
            ...
            rows: clearables.indices.map { idx in
                Row(
                    text: self.clearables[idx].clearable.label,
                    accessory: .switchToggle(
                        value: self.toggles[idx],
                        { [unowned self] checked in
