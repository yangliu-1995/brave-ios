...
class RewardsInternalsLogController: UITableViewController {
    ...
    private var lines: [LogLine] = [] {
        didSet {
            lineCountToolbarLabel.text = String.localizedStringWithFormat(
                Strings.RewardsInternals.logsCount,
                lines.count
            )
    ...
    override func viewDidLoad() {
        ...
        toolbarItems = [
            .init(
                ...
                target: self,
                action: #selector(tappedRefreshLogs)
            ),
    ...
    @objc private func tappedShare() {
        let controller = RewardsInternalsShareController(
            ledger: self.ledger,
            initiallySelectedSharables: [.logs]
        )
            ...
            do {
                ...
                for url in urls {
                    ...
                    do {
                        ...
                        let parsedLines = fileLines.suffix(availableSpace).compactMap(
                            LogLine.init(from:)
                        )
                        ...
                        browserLogger.error(
                            "Failed to load log file: \(fileURL.absoluteString) with error: \(String(describing: error))"
                        )
                    }
                }
                DispatchQueue.main.async {
                    ...
                    self?.tableView.scrollToRow(
                        at: IndexPath(row: lines.count - 1, section: 0),
                        at: .bottom,
                        animated: animated
                    )
                }
                ...
                browserLogger.error(
                    "Failed to fetch logs for Rewards: \(String(describing: error))"
                )
            }
        }
    }
...
struct RewardsInternalsLogsGenerator: RewardsInternalsFileGenerator {
    func generateFiles(
        ...
        do {
            let fileURLs = try rewardsLogger.logFilenamesAndURLs().map {
                URL(fileURLWithPath: $0.1.path)
            }
            for url in fileURLs {
                let logPath = URL(fileURLWithPath: path).appendingPathComponent(
