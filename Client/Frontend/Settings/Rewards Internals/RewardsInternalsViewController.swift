...
class RewardsInternalsViewController: TableViewController {
    ...
    func reloadSections() {
                ...
                rows: [
                    ...
                    Row(
                        ...
                        selection: { [unowned self] in
                            if let index = self.dataSource.sections[safe: 1]?.rows.firstIndex(
                                where: {
        ...
        if let legacyLedger = legacyLedger, let internals = legacyInternalsInfo,
            !legacyLedger.isLedgerTransferExpired
        {
                ...
                .init(
                    ...
                    rows: [
                        Row(
                            text: Strings.RewardsInternals.keyInfoSeed,
                            detailText:
                                "\(internals.isKeyInfoSeedValid ? Strings.RewardsInternals.valid : Strings.RewardsInternals.invalid)"
                        ),
                        Row(
                            text: Strings.RewardsInternals.walletPaymentID,
                            detailText: internals.paymentId,
                            selection: { [unowned self] in
                                if let index = self.dataSource.sections[safe: legacyWalletSection]?
                                    .rows.firstIndex(
                                        where: { $0.cellClass == PaymentIDCell.self }),
                                    let cell = self.tableView.cellForRow(
                                        at: IndexPath(item: index, section: legacyWalletSection)
                                    ) as? PaymentIDCell
                                {
                                    cell.showMenu()
                                }
                        ...
                        Row(
                            text: Strings.RewardsInternals.walletCreationDate,
                            detailText: dateFormatter.string(
                                from: Date(timeIntervalSince1970: TimeInterval(internals.bootStamp))
                            )
                        ),
                        Row(
    ...
    func generateFiles(
        ...
        let data: [String: Any] = [
            "Wallet Info": [
                ...
                "Wallet Creation Date": builder.dateFormatter.string(
                    from: Date(timeIntervalSince1970: TimeInterval(info.bootStamp))
                ),
            ],
            "Device Info": [
