...
class SettingsViewController: TableViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if let indexPath = dataSource.indexPath(
            rowUUID: vpnRow.uuid,
            sectionUUID: featuresSection.uuid
        ) {
    ...
    private func displayRewardsDebugMenu() {
        ...
        let settings = RewardsDebugSettingsViewController(
            rewards: rewards,
        ...
        var section = Section(
            ...
            rows: [
                Row(
                    ...
                    image: #imageLiteral(resourceName: "settings-shields"),
        ...
        if BraveRewards.isAvailable, let rewards = rewards {
            section.rows += [
                Row(
                    ...
                    selection: { [unowned self] in
                        let rewardsVC = BraveRewardsSettingsViewController(
                            rewards,
                            ...
                            self.dismiss(animated: true) {
                                self.presentingViewController?.dismiss(animated: true) {
                                    self.settingsDelegate?.settingsOpenURLInNewTab(
                                        BraveUX.braveRewardsLearnMoreURL
                                    )
                                }
                            }
                        }
                    ...
                    image: #imageLiteral(resourceName: "settings-brave-rewards"),
                    accessory: .disclosureIndicator
                )
            ]
        }
            ...
            section.rows.append(
                Row(
                    ...
                    selection: {
                        let todaySettings = BraveTodaySettingsViewController(
                            dataSource: self.feedDataSource
                        )
                        self.navigationController?.pushViewController(todaySettings, animated: true)
                    },
                    image: #imageLiteral(resourceName: "settings-brave-today").template,
                    accessory: .disclosureIndicator
                )
            )
        #endif
        ...
        var general = Section(
            ...
            rows: [
                Row(
                    ...
                    image: #imageLiteral(resourceName: "settings-search").template,
                    accessory: .disclosureIndicator,
                ...
                Row(
                    ...
                    selection: { [unowned self] in
                        if BraveSyncAPI.shared.isInSyncGroup {
                            ...
                            self.navigationController?
                                .pushViewController(
                                    SyncSettingsTableViewController(style: .grouped),
                                    animated: true
                                )
                        } else {
                            self.navigationController?.pushViewController(
                    ...
                    image: #imageLiteral(resourceName: "settings-sync").template,
                    accessory: .disclosureIndicator,
                    cellClass: MultilineValue1Cell.self
                ),
                .boolRow(
    ...
    private lazy var displaySection: Section = {
        ...
        let themeSubtitle = Theme.DefaultTheme(rawValue: Preferences.General.themeNormalMode.value)?
            .displayString
        var row = Row(
        ...
        row.selection = { [unowned self] in
            let optionsViewController = OptionSelectionViewController<Theme.DefaultTheme>(
                ...
                selectedOption: Theme.DefaultTheme(
        ...
        if UIDevice.current.userInterfaceIdiom == .pad {
            display.rows.append(
                Row(
                    ...
                    accessory: .switchToggle(
                        value: Preferences.General.tabBarVisibility.value
                            == TabBarVisibility.always.rawValue,
                        {
                            Preferences.General.tabBarVisibility.value =
                                $0
                                ? TabBarVisibility.always.rawValue : TabBarVisibility.never.rawValue
                        }
                    ),
                    cellClass: MultilineValue1Cell.self
                )
            ...
            row.selection = { [unowned self] in
                ...
                let optionsViewController = OptionSelectionViewController<TabBarVisibility>(
                    ...
                    selectedOption: TabBarVisibility(
    ...
    private func vpnSettingsRow() -> Row {
        ...
        let (text, color) = { () -> (String, UIColor) in
            ...
            case .installed(let enabled):
                if enabled {
                    return (
                        Strings.VPN.settingsVPNEnabled,
                        #colorLiteral(
                            red: 0.1607843137,
                            green: 0.737254902,
                            blue: 0.5647058824,
        ...
        return Row(
            ...
            image: #imageLiteral(resourceName: "settings-vpn").template,
            accessory: .disclosureIndicator,
    ...
    private lazy var securitySection: Section = {
        ...
        return Section(
            ...
            rows: [
                ...
                .boolRow(
                    title: Strings.saveLogins,
                    option: Preferences.General.saveLogins,
                    image: #imageLiteral(resourceName: "settings-save-logins").template
                ),
            ]
        )
    }()
                    ...
                    selection: { [unowned self] in
                        ...
                        guard
                            let writeReviewURL = URL(
                                string:
                                    "https://itunes.apple.com/app/id1052879175?action=write-review"
                            )
                        else { return }
        ...
        return Section(
            ...
            rows: [
                ...
                Row(
                    ...
                    selection: { [unowned self] in
                        // Show privacy policy
                        let privacy = SettingsContentViewController().then {
                            $0.url = BraveUX.bravePrivacyURL
                        }
                ...
                Row(
                    ...
                    selection: { [unowned self] in
                        // Show terms of use
                        let toc = SettingsContentViewController().then {
                            $0.url = BraveUX.braveTermsOfUseURL
                        }
                ...
                Row(
                    ...
                    selection: { [unowned self] in
                        ...
                        let licenses = SettingsContentViewController().then {
                            $0.url = url.appendingPathComponent("about").appendingPathComponent(
                                "license"
                            )
                        }
    ...
    private lazy var debugSection: Section? = {
        ...
        return Section(
            rows: [
                ...
                Row(
                    text: "View Brave Today Debug Menu",
                    selection: { [unowned self] in
...
extension TableViewController: Themeable {
    func applyTheme(_ theme: Theme) {
        ...
        navigationController?.navigationBar.barTintColor =
            UINavigationBar.appearance().appearanceBarTintColor
