...
class ShareExtensionHelper: NSObject {
    ...
    func createActivityViewController(
        ...
        _ completionHandler: @escaping (
            _ completed: Bool, _ activityType: UIActivity.ActivityType?, _ documentURL: URL?
        ) -> Void
    ...
    func writeWebPagePDFDataToURL(_ completion: @escaping (URL?, Error?) -> Void) {
        #if compiler(>=5.3)
            if #available(iOS 14.0, *), let webView = selectedTab?.webView,
                selectedTab?.temporaryDocument == nil
            {
...
extension ShareExtensionHelper: UIActivityItemSource {
    ...
    func activityViewController(
        ...
        let selectedURLItem =
            selectedURL.isReaderModeURL ? selectedURL.decodeReaderModeURL : selectedURL
...
extension ShareExtensionHelper {
    ...
    private func shareActivityType(_ activityType: String?) -> ShareActivityType {
        ...
        let isPasswordManagerType =
            (activityType?.range(of: "password") != nil)
        ...
        let isOpenInIBooksActivityType =
            (activityType?.range(of: "OpenInIBooks") != nil)
            || (activityType == "com.apple.UIKit.activity.OpenInIBooks")

        let isOpenByCopy =
            activityType?.lowercased().range(of: "remoteopeninapplication-bycopy") != nil
