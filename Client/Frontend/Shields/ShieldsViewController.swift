...
class ShieldsViewController: UIViewController, PopoverContentComponent, Themeable {
    ...
    private func updateToggleStatus() {
        ...
        if let domain = domain {
            shieldsUpSwitch.isOn = !domain.isShieldExpected(
                .AllOff,
        ...
        shieldControlMapping.forEach { shield, view, option in
            ...
            if let domain = domain {
                ...
                view.toggleSwitch.isOn = domain.isShieldExpected(
                    shield,
                    considerAllShieldsOption: false
                )
            }
    ...
    private func updateBraveShieldState(
        shield: BraveShield,
        on: Bool,
    ...
    private lazy var shieldControlMapping:
        [(BraveShield, AdvancedShieldsView.ToggleView, Preferences.Option<Bool>?)] = [
            (
                .AdblockAndTp, shieldsView.advancedShieldView.adsTrackersControl,
                Preferences.Shields.blockAdsAndTracking
            ),
            (
                .SafeBrowsing, shieldsView.advancedShieldView.blockMalwareControl,
                Preferences.Shields.blockPhishingAndMalware
            ),
            (
                .NoScript, shieldsView.advancedShieldView.blockScriptsControl,
                Preferences.Shields.blockScripts
            ),
            (
                .HTTPSE, shieldsView.advancedShieldView.httpsUpgradesControl,
                Preferences.Shields.httpsEverywhere
            ),
            (
                .FpProtection, shieldsView.advancedShieldView.fingerprintingControl,
                Preferences.Shields.fingerprintingProtection
            ),
        ]
    ...
    override func viewDidLoad() {
        ...
        shieldsView.advancedShieldView.siteTitle.titleLabel.text = url?.normalizedHost()?
            .uppercased()
