...
class SimpleShieldsView: UIView, Themeable {
    ...
    class BlockCountView: UIView, Themeable {
        ...
        let shareButton = Button().then {
            $0.setImage(
                #imageLiteral(resourceName: "shields-share").withRenderingMode(.alwaysTemplate),
                for: .normal
            )
            $0.hitTestSlop = UX.hitBoxEdgeInsets
            $0.imageEdgeInsets = .zero
        ...
        func applyTheme(_ theme: Theme) {
            ...
            let contentBackgroundColor =
                theme.isDark
                ? #colorLiteral(red: 0.1882352941, green: 0.2039215686, blue: 0.262745098, alpha: 1)
                : Colors.neutral000
    ...
    override init(frame: CGRect) {
        ...
        [shieldsDownDisclaimerLabel, reportSiteButton].forEach(
            shieldsDownStackView.addArrangedSubview
        )
    ...
    func applyTheme(_ theme: Theme) {
        shieldsSwitch.offBackgroundColor =
            theme.isDark ? UIColor(rgb: 0x26262E) : UIColor(white: 0.9, alpha: 1.0)
