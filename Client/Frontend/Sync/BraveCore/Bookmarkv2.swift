...
extension Bookmarkv2 {
    ...
    public static func foldersFrc(excludedFolder: Bookmarkv2? = nil)
        -> BookmarksV2FetchResultsController
    {
        return Bookmarkv2ExclusiveFetcher(
            excludedFolder?.bookmarkNode,
            api: Bookmarkv2.bookmarksAPI
        )
    }

    public static func getChildren(forFolder folder: Bookmarkv2, includeFolders: Bool)
        -> [Bookmarkv2]?
    {
...
extension Bookmarkv2 {
    ...
    public static func waitForBookmarkModelLoaded(_ completion: @escaping () -> Void) {
        if bookmarksAPI.isLoaded {
            ...
            observer = Bookmarkv2.bookmarksAPI.add(
                BookmarkModelStateObserver({
