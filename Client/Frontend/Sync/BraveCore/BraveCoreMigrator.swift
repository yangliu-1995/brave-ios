...
class BraveCoreMigrator {
        ...
        if Preferences.Chromium.syncV2BookmarksMigrationCompleted.value {
            ...
            self.testMassiveMigration { [weak self] in
                ...
                if self.bookmarksAPI.isLoaded {
                    ...
                    self.observer = self.bookmarksAPI.add(
    ...
    public func observeState(
        ...
        _ handler: @escaping (MigrationState, MigrationState) -> Void
    ) {
    ...
    public static var datedBookmarksURL: URL? {
        ...
        let dateString =
            dateFormatter.string(from: Date()).escape() ?? "\(Date().timeIntervalSince1970)"
    ...
    public func migrate(_ completion: ((_ success: Bool) -> Void)? = nil) {
                        ...
                        self?.migrateBookmarks { success in
                            ...
                            if let url = BraveCoreMigrator.bookmarksURL {
                                do {
                                    ...
                                    log.error(
                                        "Failed to delete Bookmarks.html backup during Migration"
                                    )
                                }
                            }
        ...
        if bookmarksAPI.isLoaded {
            ...
            self.observer = bookmarksAPI.add(
                BookmarksModelLoadedObserver({ [weak self] in
                    guard let self = self else { return }
                    self.observer?.destroy()
                    self.observer = nil
    ...
    public func exportBookmarks(to url: URL, _ completion: @escaping (_ success: Bool) -> Void) {
        self.dataImportExporter.exportBookmarks(
            to: url,
            bookmarks: LegacyBookmarksHelper.getTopLevelLegacyBookmarks().sorted(by: {
                $0.order < $1.order
            })
        ) { success in
            completion(success)
    ...
    private func migrateBookmarks(_ completion: @escaping (_ success: Bool) -> Void) {
        ...
        DataController.performOnMainContext { context in
            var didSucceed = true
            for bookmark in LegacyBookmarksHelper.getTopLevelLegacyBookmarks(context).sorted(by: {
                $0.order < $1.order
            }) {
                if self.migrateChromiumBookmarks(
                    context: context,
                    bookmark: bookmark,
                    chromiumBookmark: rootFolder
                ) {
    ...
    private func migrateChromiumBookmarks(
        ...
        if bookmark.isFolder {
            ...
            for childBookmark in bookmark.children?.sorted(by: { $0.order < $1.order }) ?? [] {
                if migrateChromiumBookmarks(
                    context: context,
                    bookmark: childBookmark,
                    chromiumBookmark: folder
                ) {
