...
class BookmarksInterstitialPageHandler {
    ...
    class func showBookmarksPage(tabManager: TabManager, url: URL) -> Bool {
        ...
        let components = URLComponents(
            string: WebServer.sharedInstance.base + "/interstitial/Bookmarks.html"
        )!
...
extension BookmarksInterstitialPageHandler {
            ...
            webServer.registerHandlerForMethod(
                ...
                handler: { (request) -> GCDWebServerResponse? in
                    guard let url = request?.url,
                        request?.headers["X-REQUEST-INTERSTITIAL-INFO"]?.isEmpty == false
                    else {
                        return GCDWebServerResponse(statusCode: 404)
                    }

                    return BookmarksInterstitialPageHandler.responseForURL(
                        url,
    ...
    private static func responseForURL(_ url: URL, headers: [String: String])
        -> GCDWebServerResponse?
    {
        ...
        switch key {
        case "bookmarks":
            guard let urlInfo = headers["X-REQUEST-INTERSTITIAL-URL"],
    ...
    private static func buildResponse(asset: String?, variables: [String: String])
        -> GCDWebServerResponse?
    {
