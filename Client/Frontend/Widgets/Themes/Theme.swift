...
class Theme: Equatable, Decodable {
    ...
    fileprivate static let themeDirectory = Bundle.main.resourceURL!.appendingPathComponent(
        "Themes"
    )
    ...
    struct Color: Decodable {
        ...
        struct Stat: Decodable {
            init(from decoder: Decoder) throws {
                ...
                let container = try decoder.container(
                    keyedBy: ThemeCodingKeys.ColorCodingKeys.StatCodingKeys.self
                )
        ...
        struct Tint: Decodable {
            init(from decoder: Decoder) throws {
                ...
                let container = try decoder.container(
                    keyedBy: ThemeCodingKeys.ColorCodingKeys.TintCodingKeys.self
                )
                let homeStr = try container.decode(String.self, forKey: .home)
                let headerStr = try container.decode(String.self, forKey: .header)
                let footerStr = try container.decode(String.self, forKey: .footer)
    ...
    static func from(id: String) -> Theme {
        ...
        let themePath = Theme.themeDirectory.appendingPathComponent(id).appendingPathExtension(
            "json"
        )
        .path
        guard
...
private enum ThemeCodingKeys: String, CodingKey {
    case uuid
    case title
    case url
    case description
