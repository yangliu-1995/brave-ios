...
class NetworkManager {
    ...
    func downloadResource(
        ...
        session.dataRequest(with: request) { data, response, error -> Void in
            ...
            default:
                let responseEtag =
                    resourceType.isCached() ? response.allHeaderFields[etagHeader] as? String : nil

                var lastModified: TimeInterval?

                if checkLastServerSideModification,
                    let lastModifiedHeaderValue = response.allHeaderFields["Last-Modified"]
                        as? String
                {
                    ...
                    lastModified =
                        formatter.date(from: lastModifiedHeaderValue)?.timeIntervalSince1970
                }

                completion.fill(
                    .init(data: data, etag: responseEtag, lastModifiedTimestamp: lastModified)
                )
            }
        }
