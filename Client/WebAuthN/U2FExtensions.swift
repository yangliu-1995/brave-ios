...
private enum U2FErrorCodes: Int {
    case ok = 1
    case other_error, bad_request, configuration_unsupported, device_ineligible, timeout
}
...
class U2FExtensions: NSObject {
    ...
    private let touchKeyPopup = AlertPopupView(
        imageView: lottieAnimation(for: "webauth_touch_key"),
        title: Strings.touchKeyMessage,
        message: "",
    ...
    private let insertKeyPopup = AlertPopupView(
        imageView: lottieAnimation(for: "webauth_insert_key"),
        title: Strings.insertKeyMessage,
        message: "",
        titleWeight: .semibold,
        titleSize: 21
    )
    ...
    private let pinVerificationPopup = AlertPopupView(
        imageView: UIImageView(image: #imageLiteral(resourceName: "enter_pin")),
    ...
    private let verificationPendingPopup =
        AlertPopupView(
            imageView: lottieAnimation(for: "webauth_verify_key"),
            title: Strings.verificationPending,
            message: "",
            titleWeight: .semibold,
            titleSize: 21
        )
    ...
    private var observeServiceStateUpdates: Bool = false {
        didSet {
            ...
            if observeServiceStateUpdates {
                keySession.addObserver(
                    self,
                    forKeyPath: #keyPath(YKFAccessorySession.fido2Service.keyState),
                    options: [],
                    context: &U2FExtensions.observationContext
                )
                keySession.addObserver(
                    self,
                    forKeyPath: #keyPath(YKFAccessorySession.u2fService.keyState),
                    options: [],
                    context: &U2FExtensions.observationContext
                )
            } else {
                keySession.removeObserver(
                    self,
                    forKeyPath: #keyPath(YKFAccessorySession.fido2Service.keyState)
                )
                keySession.removeObserver(
    ...
    private func sendJSError() {
        ...
        case .FIDORegister:
            sendFIDORegistrationError(
                handle: handle,
                requestId: self.requestId[handle] ?? -1,
                errorCode: U2FErrorCodes.other_error
            )
        case .FIDOSign:
            sendFIDOAuthenticationError(
                handle: handle,
                requestId: self.requestId[handle] ?? -1,
                errorCode: U2FErrorCodes.other_error
            )
    ...
    func validateRPID(url: String, rpId: String) -> Bool {
        ...
        guard
            let rpIdURL = rpId.hasPrefix("http://") || rpId.hasPrefix("https://")
                ? URL(string: rpId) : URL(string: "https://" + rpId)
        else {
            return false
        }
    ...
    override func observeValue(
        ...
        switch keyPath {
        case #keyPath(YKFAccessorySession.u2fService.keyState),
            #keyPath(YKFAccessorySession.fido2Service.keyState):
            ensureMainThread {
    ...
    private func handleFIDO2Registration(handle: Int, request: WebAuthnRegisterRequest) {
        ...
        guard let url = getCurrentURL() else {
            sendFIDO2RegistrationError(
                handle: handle,
                errorName: FIDO2ErrorMessages.SecurityError.rawValue
            )
            return
        }

        let publicKey = request.publicKey
        let clientData = WebAuthnClientData(
            type: WebAuthnClientDataType.create.rawValue,
        ...
        do {
            ...
            let makeOptions = [
                YKFKeyFIDO2MakeCredentialRequestOptionRK:
                    publicKey.authenticatorSelection?.requireResidentKey ?? false
            ]
            makeCredentialRequest.options = makeOptions

            let exclusionList: [YKFFIDO2PublicKeyCredentialDescriptor] =
                publicKey.excludeCredentials?.compactMap({
            ...
            executeKeyRequest { [weak self] in
                ...
                fido2Service.execute(makeCredentialRequest) { [weak self] response, error in
                    ...
                    if self.keyType == .accessory {
                        self.finalizeFIDO2Registration(
                            handle: handle,
                            response: response,
    ...
    private func finalizeFIDO2Registration(
        handle: Int,
        response: YKFKeyFIDO2MakeCredentialResponse,
        clientDataJSON: String,
        error: NSErrorPointer
    ) {
        if error != nil {
            let errorDescription =
                error?.pointee?.localizedDescription ?? Strings.U2FRegistrationError
        ...
        ensureMainThread {
            self.tab?.webView?.evaluateSafeJavaScript(
                functionName: "fido2internal\(UserScriptManager.securityTokenString).postCreate",
                args: [
                    handle, "true", credentialIdString, attestationString, clientDataJSON, "", "",
                ],
                sandboxed: false
            ) { _, error in
                if error != nil {
                    let errorDescription =
                        error?.localizedDescription ?? U2FErrorMessages.ErrorRegistration.rawValue
    ...
    private func handleMakeCredential(handle: Int, request: WebAuthnRegisterRequest, error: Error?) {
        ...
        if makeCredentialError.code == YKFKeyFIDO2ErrorCode.CREDENTIAL_EXCLUDED.rawValue {
            sendFIDO2RegistrationError(
                handle: handle,
                errorName: FIDO2ErrorMessages.InvalidStateError.rawValue,
    ...
    private func sendFIDO2RegistrationError(
        handle: Int,
        errorName: String = FIDO2ErrorMessages.NotAllowedError.rawValue,
        errorDescription: String = Strings.U2FRegistrationError
    ) {
        cleanupFIDO2Registration(handle: handle)
        ensureMainThread {
            self.tab?.webView?.evaluateSafeJavaScript(
                functionName: "fido2internal\(UserScriptManager.securityTokenString).postCreate",
                args: [
                    handle, "true", "", "", "", errorName.toBase64(), errorDescription.toBase64(),
                ],
                sandboxed: false
            ) { _, error in
                if error != nil {
    ...
    private func handleFIDO2Authentication(handle: Int, request: WebAuthnAuthenticateRequest) {
        ...
        let clientData = WebAuthnClientData(
            type: WebAuthnClientDataType.get.rawValue,
        ...
        do {
            ...
            executeKeyRequest { [weak self] in
                ...
                fido2Service.execute(getAssertionRequest) { [weak self] response, error in
                    ...
                    if self.keyType == .accessory {
                        self.finalizeFIDO2Authentication(
                            handle: handle,
                            response: response,
    ...
    private func finalizeFIDO2Authentication(
        handle: Int,
        response: YKFKeyFIDO2GetAssertionResponse,
        clientDataJSON: Data,
        error: NSErrorPointer
    ) {
        if error != nil {
            let errorDescription =
                error?.pointee?.localizedDescription ?? Strings.U2FAuthenticationError
        ...
        ensureMainThread {
            self.tab?.webView?.evaluateSafeJavaScript(
                functionName: "fido2internal\(UserScriptManager.securityTokenString).postGet",
                args: [
                    handle, "true", requestId, authenticatorData, clientDataJSONString, sig,
                    userHandle,
                ],
                sandboxed: false
            ) { _, error in
    ...
    private func handleGetAssertion(
        handle: Int,
    ...
    private func sendFIDO2AuthenticationError(
        handle: Int,
        errorName: String = FIDO2ErrorMessages.NotAllowedError.rawValue,
        errorDescription: String = Strings.U2FAuthenticationError
    ) {
        cleanupFIDO2Authentication(handle: handle)
        ensureMainThread {
            self.tab?.webView?.evaluateSafeJavaScript(
                functionName: "fido2internal\(UserScriptManager.securityTokenString).postGet",
                args: [
                    handle, "true", "", "", "", "", "", errorName.toBase64(),
                    errorDescription.toBase64(),
                ],
                sandboxed: false
            ) { _, error in
                if error != nil {
                    let errorDescription =
                        error?.localizedDescription ?? U2FErrorMessages.ErrorAuthentication.rawValue
                    log.error(errorDescription)
                }
            }
        }
    ...
    private func presentInteractWithKeyModal() {
        ...
        if u2fActive && keyType == .accessory && tab?.id == currentTabId
            && (fido2Service.keyState == .touchKey
                || u2fService.keyState == .YKFKeyU2FServiceKeyStateTouchKey)
        {
            let currentURL = self.tab?.url?.host ?? ""
    ...
    private func handlePinVerificationRequired(completion: @escaping (Bool) -> Void) {
        ensureMainThread {
            let currentURL = self.tab?.url?.host ?? ""
            self.pinVerificationPopup.addButton(title: Strings.confirmPin, type: .primary) {
                [weak self] in
                guard let self = self else {
                    completion(false)
                    return .flyDown
                ...
                if self.keyType == .accessory {
                    self.verificationPendingPopup.update(
                        title: Strings.verificationPending + currentURL
                    )
    ...
    private func verifyPin(completion: @escaping (Bool) -> Void) -> PopupViewDismissType {
        guard
            ...
            let verifyPinRequest = YKFKeyFIDO2VerifyPinRequest(pin: pin)
        else {
            completion(true)
            return .flyDown
        }
    ...
    private func requestFIDORegistration(handle: Int, requests: [FIDORegisterRequest], id: Int) {
        ...
        guard let request = requests.first else {
            sendFIDORegistrationError(
                handle: handle,
                requestId: id,
                errorCode: U2FErrorCodes.bad_request
            )
            return
        }
    ...
    private func handleFIDORegistration(handle: Int, request: FIDORegisterRequest, requestId: Int) {
        ...
        guard
            let registerRequest = YKFKeyU2FRegisterRequest(
                challenge: request.challenge,
                appId: request.appId ?? ""
            )
        else {
        ...
        executeKeyRequest { [weak self] in
            ...
            guard let u2fservice = self.keyU2fService else {
                self.sendFIDORegistrationError(
                    handle: handle,
                    requestId: requestId,
                    errorCode: U2FErrorCodes.other_error
                )
            ...
            u2fservice.execute(registerRequest) { [weak self] response, error in
                ...
                guard let clientData = response?.clientData.websafeBase64String(),
                    let registrationData = response?.registrationData.websafeBase64String()
                else {
                    self.sendFIDORegistrationError(
                        handle: handle,
                        requestId: requestId,
    ...
    private func finalizeFIDORegistration(
        requestId: Int,
        handle: Int,
        version: String,
        ...
        if requestId >= 0 {
            ensureMainThread {
                self.tab?.webView?.evaluateSafeJavaScript(
                    functionName:
                        "fidointernal\(UserScriptManager.securityTokenString).postLowLevelRegister",
                    args: [
                        requestId, "true", version, registrationData, clientData, defaultErrorCode,
                        "",
                    ],
                    sandboxed: false
                ) { _, error in
        ...
        ensureMainThread {
            self.tab?.webView?.evaluateSafeJavaScript(
                functionName: "fidointernal\(UserScriptManager.securityTokenString).postRegister",
                args: [
                    handle, "true", version, registrationData, clientData, defaultErrorCode, "",
                ],
                sandboxed: false
            ) { _, error in
                if error != nil {
    ...
    private func sendFIDORegistrationError(
        handle: Int,
        requestId: Int,
        errorCode: U2FErrorCodes,
        errorMessage: String = Strings.U2FRegistrationError
    ) {
        cleanupFIDORegistration(handle: handle)
        if requestId >= 0 {
            ensureMainThread {
                self.tab?.webView?.evaluateSafeJavaScript(
                    functionName:
                        "fidointernal\(UserScriptManager.securityTokenString).postLowLevelRegister",
                    args: [
                        requestId, "true", "", "", "", errorCode.rawValue, errorMessage.toBase64(),
                    ],
                    sandboxed: false
                ) { _, error in
                    if error != nil {
        ...
        ensureMainThread {
            self.tab?.webView?.evaluateSafeJavaScript(
                functionName: "fidointernal\(UserScriptManager.securityTokenString).postRegister",
                args: [handle, "true", "", "", "", errorCode.rawValue, errorMessage.toBase64()],
                sandboxed: false
            ) { _, error in
                if error != nil {
                    let errorDescription =
    ...
    private func handleFIDOAuthentication(handle: Int, keys: [FIDOSignRequest], requestId: Int) {
        ...
        for key in keys {
            guard
                let signRequest = YKFKeyU2FSignRequest(
            ...
            executeKeyRequest { [weak self] in
                ...
                guard let u2fservice = self.keyU2fService else {
                    self.sendFIDOAuthenticationError(
                        handle: handle,
                        requestId: requestId,
                        errorCode: U2FErrorCodes.other_error
                    )
                ...
                u2fservice.execute(signRequest) { [weak self] response, error in
                    ...
                    if error != nil {
                        if requestsCount == 0 {
                            let errorMessage =
                                error?.localizedDescription ?? Strings.U2FAuthenticationError
                            self.sendFIDOAuthenticationError(
                                handle: handle,
                                requestId: requestId,
                                errorCode: U2FErrorCodes.other_error,
                    ...
                    if self.keyType == .accessory {
                        self.finalizeFIDOAuthentication(
                            requestId: requestId,
                            handle: handle,
                            keyHandle: keyHandle,
                            signature: signature,
    ...
    private func finalizeFIDOAuthentication(
        requestId: Int,
        handle: Int,
        keyHandle: String,
        signature: String,
        ...
        if requestId >= 0 {
            ensureMainThread {
                self.tab?.webView?.evaluateSafeJavaScript(
                    functionName:
                        "fidointernal\(UserScriptManager.securityTokenString).postLowLevelSign",
                    args: [
                        requestId, "true", keyHandle, signature, clientData, defaultErrorCode, "",
                    ],
                    sandboxed: false
                ) { _, error in
                    if error != nil {
        ...
        ensureMainThread {
            self.tab?.webView?.evaluateSafeJavaScript(
                functionName: "fidointernal\(UserScriptManager.securityTokenString).postSign",
                args: [handle, "true", keyHandle, signature, clientData, defaultErrorCode, ""],
                sandboxed: false
            ) { _, error in
                if error != nil {
                    let errorDescription =
    ...
    private func sendFIDOAuthenticationError(
        handle: Int,
        requestId: Int,
        errorCode: U2FErrorCodes,
        errorMessage: String = Strings.U2FAuthenticationError
    ) {
        cleanupFIDOAuthentication(handle: handle)
    ...
    private func executeKeyRequest(execution: @escaping () -> Void) {
        if keyType == .accessory {
            ...
            nfcSesionStateObservation = nfcSession.observe(
                \.iso7816SessionState,
                changeHandler: { [weak self] session, change in
                    guard let self = self else {
                        log.error(U2FErrorMessages.Error.rawValue)
        ...
        if keyType == .nfc || accessoryState == .open {  // The key session is ready to be used.
            ...
            if !fidoRegHandles.isEmpty {
                ...
                guard let request = fidoRegisterRequest[handle] else {
                    sendFIDORegistrationError(
                        handle: handle,
                        requestId: requestId[handle] ?? -1,
                        errorCode: U2FErrorCodes.other_error
                    )
                    return
                }
                handleFIDORegistration(
                    handle: handle,
                    request: request,
                    requestId: requestId[handle] ?? -1
                )
...
extension U2FExtensions: TabContentScript {
    ...
    func userContentController(
        ...
        if message.name == "U2F\(UserScriptManager.messageHandlerTokenString)",
            let body = message.body as? NSDictionary
        {
            ...
            case U2FMessageType.FIDO2Create.rawValue:
                ...
                guard let data = body["data"] as? String,
            ...
            case U2FMessageType.FIDO2Get.rawValue:
                ...
                do {
                    let request = try JSONDecoder().decode(
                        WebAuthnAuthenticateRequest.self,
                        from: jsonData
                    )
            ...
            case U2FMessageType.FIDORegister.rawValue:
                ...
                guard let appId = body["appId"] as? String,
                    let requests = body["requests"] as? String
                else {
                    sendFIDORegistrationError(
                        handle: handle,
                        requestId: -1,
                ...
                do {
                    var registerRequests = try decoder.decode(
                        [FIDORegisterRequest].self,
                        from: jsonData
                    )
                ...
                do {
                    ...
                    if lowLevelRequest.type == registerRequest {
                        let request = try decoder.decode(
                            FIDOLowLevelRegisterRequests.self,
                            from: jsonData
                        )
                        ...
                        requestFIDORegistration(
                            handle: handle,
                            requests: registerRequests,
                            id: lowLevelRequest.requestId
                        )
                    }
                    if lowLevelRequest.type == signRequest {
                        let request = try decoder.decode(
                            FIDOLowLevelSignRequests.self,
                            from: jsonData
                        )
...
extension Strings {
    ...
    public static let tryAgain = NSLocalizedString(
        ...
        comment: "Suffix for error strings"
    )
    public static let U2FRegistrationError =
        NSLocalizedString(
            "U2FRegistrationError",
            tableName: "BraveShared",
            bundle: Bundle.braveShared,
            value: "Error registering your security key",
            comment: "Error handling U2F registration."
        )
        + tryAgain
    public static let U2FAuthenticationError =
        NSLocalizedString(
            "U2FAuthenticationError",
            tableName: "BraveShared",
            bundle: Bundle.braveShared,
            value: "Error authenticating your security key",
            comment: "Error handling U2F authentication."
        ) + tryAgain

    // Lightning Modals
    public static let touchKeyMessage = NSLocalizedString(
        "touchKeyMessage",
        bundle: Bundle.shared,
        value: "Touch your key to finish the request for ",
        comment: "Message for touch key modal."
    )
    public static let insertKeyMessage = NSLocalizedString(
        "insertKeyMessage",
        bundle: Bundle.shared,
        value: "Insert your security key for ",
        comment: "Message for touch key modal."
    )
    public static let keyCancel = NSLocalizedString(
        "touchKeyCancel",
        bundle: Bundle.shared,
        value: "Cancel",
        comment: "Text for touch key modal button."
    )
    public static let selectKey = NSLocalizedString(
        "selectKey",
        bundle: Bundle.shared,
        value: "Scan or Insert your security key for ",
        comment: "Message for selecting security key."
    )

    // PIN
    public static let pinTitle = NSLocalizedString(
        "pinTitle",
        bundle: Bundle.shared,
        value: "PIN verification required",
        comment: "Title for the alert modal when a security key with PIN is inserted."
    )
    public static let pinPlaceholder = NSLocalizedString(
        "pinPlaceholder",
        bundle: Bundle.shared,
        value: "Enter your PIN",
        comment: "Placeholder text for PIN"
    )
    public static let confirmPin = NSLocalizedString(
        "confirmPin",
        bundle: Bundle.shared,
        value: "Verify",
        comment: "Button text to confirm PIN"
    )
    public static let verificationPending = NSLocalizedString(
        "verificationPending",
        bundle: Bundle.shared,
        value: "Verifying your PIN for ",
        comment: "Title when PIN code is being verified."
    )
}
