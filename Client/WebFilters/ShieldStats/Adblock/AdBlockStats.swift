...
class AdBlockStats: LocalAdblockResourceProtocol {
    ...
    fileprivate var isRegionalAdblockEnabled: Bool {
        return Preferences.Shields.useRegionAdBlock.value
    }
    ...
    private func parseBundledGeneralBlocklist() {
        guard let path = Bundle.main.path(forResource: bundledGeneralBlocklist, ofType: "dat")
        else {
    ...
    @discardableResult func setDataFile(data: Data, id: String) -> Deferred<()> {
        ...
        guard let engine = isGeneralAdblocker(id: id) ? generalAdblockEngine : regionalAdblockEngine
        else {
