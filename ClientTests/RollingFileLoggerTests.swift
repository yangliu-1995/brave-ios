...
class RollingFileLoggerTests: XCTestCase {
    ...
    func testNewLogCreatesLogFileWithTimestamp() {
        ...
        XCTAssertTrue(
            logString!.contains(testMessage),
            "Log should contain our test message that we wrote"
        )
    }
    ...
    func testNewLogDeletesPreviousLogIfItsTooLarge() {
        ...
        let directorySize = try! manager.getAllocatedSizeOfDirectoryAtURL(
            dirURL,
        ...
        XCTAssertGreaterThan(
            directorySize,
            Int64(sizeLimit),
            "Log folder should be larger than size limit"
        )
    ...
    func testNewLogDeletesOldestLogFileToMakeRoomForNewFile() {
        ...
        let logFilePaths = [0, 1, 2, 3, 4].map {
            self.createNewLogFileWithSize(200, withDate: Date().addingTimeInterval(60 * 60 * $0))
        }
        .sorted { $0 < $1 }
        ...
        XCTAssertTrue(manager.fileExists(atPath: newExpectedPath), "New log file should exist")
        XCTAssertTrue(
            manager.fileExists(atPath: logFilePaths.first!),
            "Old log file exists until pruned"
        )
        logger.deleteOldLogsDownToSizeLimit()
        XCTAssertFalse(
            manager.fileExists(atPath: logFilePaths.first!),
            "Oldest log file should NOT exist"
        )
    }
