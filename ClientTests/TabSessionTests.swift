                ...
                for tab in self.tabManager.allTabs {
                    ...
                    webView.configuration.websiteDataStore.fetchDataRecords(
                        ...
                        completionHandler: { records in
                            XCTAssertFalse(
                                records.isEmpty,
                                "Error: Data Store not shared amongst private tabs!"
                            )
                            ...
                            XCTAssertTrue(
                                ...
                                "Data Store records do not match!"
                            )
                            group.leave()
                        }
                    )
    ...
    func testNormalTabSessionSharing() {
        ...
        destroyData {
            ...
            self.tabManager.removeTabs(
                self.tabManager.allTabs.filter({
                    $0.url?.absoluteString.contains("localhost") ?? false
                })
            )
            if self.tabManager.allTabs.count != 4 {
                XCTFail("Error: Not all Tabs are created equally")
                ...
                for tab in self.tabManager.allTabs {
                    ...
                    webView.configuration.websiteDataStore.fetchDataRecords(
                        ...
                        completionHandler: { records in
                            XCTAssertFalse(
                                records.isEmpty,
                                "Error: Data Store not shared amongst normal tabs!"
                            )
                    ...
                    for tab in self.tabManager.tabs(withType: .regular) {
                        ...
                        webView.configuration.websiteDataStore.fetchDataRecords(
                            ...
                            completionHandler: { records in
                                XCTAssertFalse(
                                    records.isEmpty,
                                    "Error: Data Store not persistent in normal mode!"
                                )
