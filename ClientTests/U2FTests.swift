...
class U2FTests: XCTestCase {
    ...
    func testWebAuthnRegisterRequest() {
        let webAuthnRegisterData =
            "{\"publicKey\":{\"attestation\":\"direct\",\"authenticatorSelection\":{\"requireResidentKey\":false,\"userVerification\":\"discouraged\"},\"pubKeyCredParams\":[{\"alg\":-7,\"type\":\"public-key\"},{\"alg\":-257,\"type\":\"public-key\"}],\"rp\":{\"id\":\"demo.brave.com\",\"name\":\"Brave\"},\"timeout\":90000,\"challenge\": \"mdMbxTPACurawWFHqkoltSUwDear2OZQVl/uhBNqiaM=\",\"user\":{\"displayName\":\"Brave demo user\",\"name\":\"Brave demo user\",\"id\":\"OvQO5490o1w89Op/9dp4w7VvKuLEk5NHcfOnc2ZECtc=\"},\"excludeCredentials\":[]},\"signal\":{}}"
        ...
        do {
            ...
            XCTAssertEqual(
                publicKey.user.id,
                "OvQO5490o1w89Op/9dp4w7VvKuLEk5NHcfOnc2ZECtc=",
                "request user id is correct."
            )
            ...
            XCTAssertEqual(
                publicKey.challenge,
                "mdMbxTPACurawWFHqkoltSUwDear2OZQVl/uhBNqiaM=",
                "request challenge is correct."
            )
        } catch {
            XCTFail("\(error)")
        }
    ...
    func testWebAuthnAuthenticateRequest() {
        let webAuthnAuthenticateData =
            "{\"publicKey\":{\"allowCredentials\":[{\"type\":\"public-key\",\"id\":\"OvQO5490o1w89Op/9dp4w7VvKuLEk5NHcfOnc2ZECtc=\"}],\"rpId\":\"demo.brave.com\",\"timeout\":90000,\"userVerification\":\"discouraged\",\"challenge\":\"mdMbxTPACurawWFHqkoltSUwDear2OZQVl/uhBNqiaM=\"},\"signal\":{}}"
        ...
        do {
            ...
            XCTAssertEqual(
                request.allowCredentials.count,
                1,
                "request allowCredential count is correct"
            )
            XCTAssertEqual(
                request.allowCredentials.first,
                "OvQO5490o1w89Op/9dp4w7VvKuLEk5NHcfOnc2ZECtc=",
                "request allowCredential is correct"
            )
    ...
    func testWebAuthnUserVerification() {
        do {
            ...
            let expected: [WebAuthnUserVerification] = [
                .required, .preferred, .discouraged, .preferred,
            ]

            // Test expected verifications
            for verification in verifications {
                let payload =
                    "{\"publicKey\":{\"allowCredentials\":[{\"type\":\"public-key\",\"id\":\"OvQO5490o1w89Op/9dp4w7VvKuLEk5NHcfOnc2ZECtc=\"}],\"rpId\":\"demo.brave.com\",\"timeout\":90000,\"userVerification\":\"\(verification)\",\"challenge\":\"mdMbxTPACurawWFHqkoltSUwDear2OZQVl/uhBNqiaM=\"},\"signal\":{}}"
                payloads.append(payload)
            }

            // Test when userPresence is MISSING from the payload..
            payloads.append(
                "{\"publicKey\":{\"allowCredentials\":[{\"type\":\"public-key\",\"id\":\"OvQO5490o1w89Op/9dp4w7VvKuLEk5NHcfOnc2ZECtc=\"}],\"rpId\":\"demo.brave.com\",\"timeout\":90000,\"challenge\":\"mdMbxTPACurawWFHqkoltSUwDear2OZQVl/uhBNqiaM=\"},\"signal\":{}}"
            )
            ...
            for i in 0..<payloads.count {
                ...
                XCTAssertTrue(
                    request.userVerification == expected[i],
                    "request userPresence is correct"
                )
            }
        } catch {
            XCTFail("\(error)")
    ...
    func testValidateRPId() {
        ...
        XCTAssertTrue(
            U2FExtension.validateRPID(url: currentURL.absoluteString, rpId: rpIdURLs.removeFirst()),
            "rpID URL is valid."
        )
        XCTAssertTrue(
            U2FExtension.validateRPID(url: currentURL.absoluteString, rpId: rpIdURLs.removeFirst()),
        ...
        for url in rpIdURLs {
            XCTAssertFalse(
                U2FExtension.validateRPID(url: currentURL.absoluteString, rpId: url),
                "rpID URLs is invalid."
            )
        }
    }
}
