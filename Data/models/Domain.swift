...
public final class Domain: NSManagedObject, CRUD {
    ...
    public class func setBraveShield(
        ...
        let _context: WriteContext =
            isPrivateBrowsing ? .new(inMemory: true) : .new(inMemory: false)
    ...
    public func isShieldExpected(_ shield: BraveShield, considerAllShieldsOption: Bool) -> Bool {
        let isShieldOn = { () -> Bool in
            ...
            case .SafeBrowsing:
                return self.shield_safeBrowsing?.boolValue
                    ?? Preferences.Shields.blockPhishingAndMalware.value
            case .FpProtection:
        ...
        return considerAllShieldsOption
            ? !isAllShieldsOff && isSpecificShieldOn : isSpecificShieldOn
    }
    ...
    public static func migrateShieldOverrides() {
        ...
        DataController.perform { context in
            ...
            for domain in httpDomains {
                ...
                if let httpsDomain = Domain.first(
                    where: NSPredicate(format: "url == %@", httpsUrl),
                    context: context
                ) {
...
extension Domain {
    ...
    class func getOrCreateInternal(
        ...
        let writeContext =
            ...
            ? saveStrategy.saveContext : context
        ...
        guard let domainOnCorrectContext = context.object(with: newDomain.objectID) as? Domain
        else {
    ...
    private func domainForInverseHttpScheme(context: NSManagedObjectContext) -> Domain? {
        ...
        return Domain.getOrCreateInternal(
            url,
            context: context,
            saveStrategy: .delayedPersistentStore
        )
