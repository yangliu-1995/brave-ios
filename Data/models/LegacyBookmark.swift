...
extension Favorite {
    ...
    fileprivate static func restore_1_12_Bookmarks(completion: @escaping () -> Void) {
        ...
        guard let oldBookmarks = Favorite.all(context: context)?.sorted() else {
            ...
            log.warning(
                "Could not get bookmarks from database. Will retry restoration on next app launch."
            )
            return
        }
    ...
    private static func restoreLostBookmarksInternal(
        _ bookmarksToRestore: [Favorite],
        ...
        for restoredBookmark in bookmarksToRestore {
            ...
            if restoredBookmark.isFolder {
                ...
                bookmarkType =
                    restoredBookmark.isFavorite ? .favorite(url: bUrl) : .bookmark(url: bUrl)
            }

            guard let type = bookmarkType else { continue }
            let bookmarkData = BookmarkRestorationData(
                bookmarkType: type,
                id: restoredBookmark.objectID,
    ...
    private static func reinsertBookmarks(
        saveLocation: Favorite?,
        ...
        bookmarksToInsertAtGivenLevel.forEach { bookmark in
            ...
            case .folder:
                ...
                self.reinsertBookmarks(
                    saveLocation: nil,
                    bookmarksToInsertAtGivenLevel: children,
...
extension Favorite {
    ...
    private class func migrateOrder(
        parentFolder: Favorite? = nil,
        forFavorites: Bool,
        context: NSManagedObjectContext
    ) {
    ...
    private class func allBookmarksOfAGivenLevelPredicate(parent: Favorite?) -> NSPredicate {
        ...
        return NSPredicate(
            format: "%K == %@ AND %K == NO",
            parentFolderKP,
            parent ?? nilArgumentForPredicate,
            isFavoriteKP
        )
    }
}
