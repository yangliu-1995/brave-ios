...
class CRUDProtocolsTests: CoreDataTestCase {
    ...
    let fetchRequest = NSFetchRequest<NSFetchRequestResult>(
        entityName: String(describing: CRUDClass.self)
    )

    private func entity(for context: NSManagedObjectContext) -> NSEntityDescription {
        return NSEntityDescription.entity(
            forEntityName: String(describing: CRUDClass.self),
            in: context
        )!
    }
    ...
    func testDelete() {
        ...
        XCTAssertEqual(
            try! DataController.viewContext.count(for: fetchRequest),
            initialObjectsCount - 1
        )
