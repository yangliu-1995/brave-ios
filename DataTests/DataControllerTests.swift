...
class DataControllerTests: CoreDataTestCase {
    ...
    private func entity(for context: NSManagedObjectContext) -> NSEntityDescription {
        return NSEntityDescription.entity(
            forEntityName: String(describing: "Bookmark"),
            in: context
        )!
    }
    ...
    func testStoreIsEmpty() {
        ...
        let domainFR = NSFetchRequest<NSFetchRequestResult>(
            entityName: String(describing: Domain.self)
        )
        XCTAssertEqual(try! viewContext.count(for: domainFR), 0)

        let historyFR = NSFetchRequest<NSFetchRequestResult>(
