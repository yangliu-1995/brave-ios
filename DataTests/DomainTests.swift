...
class DomainTests: CoreDataTestCase {
    ...
    private func entity(for context: NSManagedObjectContext) -> NSEntityDescription {
        return NSEntityDescription.entity(
            forEntityName: String(describing: Domain.self),
            in: context
        )!
    }
    ...
    func testDefaultShieldSettings() {
        ...
        XCTAssertTrue(
            domain.isShieldExpected(BraveShield.SafeBrowsing, considerAllShieldsOption: true)
        )
        XCTAssertFalse(domain.isShieldExpected(BraveShield.AllOff, considerAllShieldsOption: true))
        XCTAssertFalse(
            domain.isShieldExpected(BraveShield.NoScript, considerAllShieldsOption: true)
        )
        XCTAssertFalse(
    ...
    func testAllShieldsOff() {
        ...
        XCTAssertTrue(
            domain.isShieldExpected(BraveShield.SafeBrowsing, considerAllShieldsOption: false)
        )
        XCTAssertFalse(domain.isShieldExpected(BraveShield.AllOff, considerAllShieldsOption: true))
        XCTAssertFalse(
            domain.isShieldExpected(BraveShield.NoScript, considerAllShieldsOption: true)
    ...
    func testHTTPSEforHTTPsetter() {
        ...
        XCTAssertEqual(
            domainRefetch1.isShieldExpected(.HTTPSE, considerAllShieldsOption: true),
            true
        )

        let domainRefetch2 = Domain.getOrCreate(forUrl: urlHTTPS, persistent: true)
        XCTAssertEqual(
            domainRefetch2.isShieldExpected(.HTTPSE, considerAllShieldsOption: true),
            true
        )
    }
