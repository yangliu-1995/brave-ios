...
extension NSCoder {
    ...
    **/
    ...
    open func decodeAsUInt64(forKey key: String) -> UInt64 {
        return (self.decodeObject(forKey: key) as? NSNumber)?.uint64Value
            ?? UInt64(self.decodeInt64(forKey: key))
    }
    ...
    open func decodeAsDouble(forKey key: String) -> Double {
        return (self.decodeObject(forKey: key) as? NSNumber)?.doubleValue
            ?? self.decodeDouble(forKey: key)
    }
}
