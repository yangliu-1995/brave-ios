...
class FeatureSwitchTests: XCTestCase {
    ...
    func testConsistentWhenChangingPercentage() {
        ...
        XCTAssertEqual(
            changed,
            1,
            "Users should get and keep the feature if the feature is becoming successful"
        )
    }
...
extension FeatureSwitchTests {
    ...
    func testAppConstantsWin() {
        ...
        let buildChannelAndFlaggedOn = FeatureSwitch(
            named: "test-flagged-on",
            true,
            allowPercentage: 0
        )
        ...
        let buildChannelAndFlaggedOff = FeatureSwitch(
            named: "test-flagged-off",
            false,
            allowPercentage: 100
        )
...
extension FeatureSwitchTests {
    fileprivate func sampleN(_ featureSwitch: FeatureSwitch, testCount: Int = 1000) -> Int {
        var count = 0
    ...
    fileprivate func testExactly(_ featureSwitch: FeatureSwitch, expected: Int) {
        let testCount = 1000
    ...
    fileprivate func testApprox(_ featureSwitch: FeatureSwitch, expected: Int, epsilon: Int = 2) {
        let testCount = 10000
        let count = sampleN(featureSwitch, testCount: testCount)
